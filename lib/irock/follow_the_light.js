...
var follow_the_light = function (white_rabbit) {
    ...
    if (!white_rabbit.imu_data.connected) {
    ...
    function pitch_speed_boost() {
        const pitch_rad = (white_rabbit.imu_data.pitch || 0) * (Math.PI / 180);
    ...
    function roll_steer_correction() {
        const roll_rad = (white_rabbit.imu_data.roll || 0) * (Math.PI / 180);
    ...
    if (!state.phase) {
        state.phase             = 'seek';
        state.ref_pitch         = white_rabbit.imu_data.pitch || 0;
        state.ramp_entered_at   = null;
        state.seek_direction    = 1;
        state.seek_switched_at  = Date.now();
        state.complete_timer    = null;
        white_rabbit.logs.irlock.log(white_rabbit, 'follow_the_light: starting (rear-center camera @ ' + camera_height_m.toFixed(2) + 'm, reverse approach) — ref_pitch=' + state.ref_pitch.toFixed(2));
    ...
    if (state.phase === 'approach' || state.phase === 'ramp') {
        ...
        const pitch       = white_rabbit.imu_data.pitch || 0;
        ...
        if (state.phase === 'approach' && Math.abs(pitch_delta) >= ramp_detect_delta) {
            ...
            white_rabbit.logs.irlock.log(white_rabbit, 'follow_the_light: ramp detected (pitch_delta=' + pitch_delta.toFixed(3) + ')');
        ...
        if (!state._last_log_ts || now - state._last_log_ts >= 1000) {
            ...
            white_rabbit.logs.irlock.log(white_rabbit,
                ...
                + ' roll=' + (white_rabbit.imu_data.roll || 0).toFixed(1) + '°'
    ...
    if (state.phase === 'complete') {
        if (!state.complete_timer) {
            ...
            state.complete_timer = setTimeout(() => {
                ...
                dock.dock_pitch     = white_rabbit.imu_data.pitch || 0;
