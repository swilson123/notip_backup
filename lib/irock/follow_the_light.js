...
//                beacon using angle_x; drive at base speed.  Pitch/roll
//                compensation inactive (ground is level).  Transitions to
//                'ramp' when pitch departs by ramp_detect_pitch_delta.
//
//   ramp       → Rover is climbing the ramp.  Pitch compensation increases
//                motor speed to maintain momentum.  Roll compensation adjusts
//                left/right balance for a tilted ramp.  Steering still driven
//                by angle_x.  Transitions to 'complete' when pitch settles
//                near the dock_pitch (top of ramp).
//
...
var follow_the_light = function (rover) {
    ...
    if (state.phase === 'approach' || state.phase === 'ramp') {
        ...
        // Safety: if beacon is lost for > 500 ms, go back to seek
        if (!fresh) {
        ...
        // Safety: if beacon fills too much of the frame, we are too close — stop
        if (rover.irlock.target && rover.irlock.target.size_norm >= size_stop_threshold) {
        ...
        // Detect ramp exit (pitch returned near reference after being on ramp)
        if (state.phase === 'ramp') {
            const on_ramp_long_enough = state.ramp_entered_at && (Date.now() - state.ramp_entered_at > 1000);
        ...
        // Compose speed and steering
        const boost   = (state.phase === 'ramp') ? pitch_speed_boost() : 0;
        const speed   = base_speed + boost;
        const steer   = steer_delta_from_beacon() + roll_steer_correction();
