var follow_the_light = function (white_rabbit) {
	...
	if (white_rabbit.robot_data.is_armed) {
		// Don't require the external IMU (it may be disabled); all pitch reads go
		// through get_pitch, which falls back to the Pixhawk ATTITUDE. Wait only for
		// a live ATTITUDE pitch reading (typeof so a level pitch of 0 isn't "missing").
		...
		if (!white_rabbit.dock.dock_state) {
			...
			if (white_rabbit.irlock.detected && white_rabbit.irlock.target) {
				// Light found — stop the search spin cleanly before handing off.
				...
				// One full sweep done — the light wasn't found in this pass.
				// Noah does not give up. He holds his position and waits with confidence.
				// The light will return. Noah knows.
				console.log("Search sweep complete — holding position, waiting for the light.");
				white_rabbit.move_white_rabbit(white_rabbit, 1, 0, "follow_the_light");
				white_rabbit.move_white_rabbit(white_rabbit, 2, 0, "follow_the_light");
				white_rabbit.move_white_rabbit(white_rabbit, 3, 0, "follow_the_light");
				white_rabbit.move_white_rabbit(white_rabbit, 4, 0, "follow_the_light");
				white_rabbit.dock.dock_state = "waiting_for_the_light";
				white_rabbit.dock._waiting_voice_ts = null;
				if (white_rabbit.voice) white_rabbit.voice.say('Holding position. Waiting for the light.');
			} else {
				// Spin clockwise in place to sweep the full circle for the beacon.
				// Mirror yaw_white_rabbit exactly: servos to spin geometry first,
				// motors only after all four servos confirm they're in position.
				...
				if (search_servos_ready) {
					...
					// Servos still moving to spin geometry — hold motors until ready.
					white_rabbit.move_white_rabbit(white_rabbit, 1, 0, "follow_the_light");
					white_rabbit.move_white_rabbit(white_rabbit, 2, 0, "follow_the_light");
					white_rabbit.move_white_rabbit(white_rabbit, 3, 0, "follow_the_light");
					white_rabbit.move_white_rabbit(white_rabbit, 4, 0, "follow_the_light");
		...
		else if (white_rabbit.dock.dock_state === "waiting_for_the_light") {

			// Noah holds his position at the undock heading and waits.
			// No fear. The light will return. He vocalizes his state every 20s
			// so Scott always knows where he is and what he is doing.
			white_rabbit.move_white_rabbit(white_rabbit, 1, 0, "follow_the_light");
			white_rabbit.move_white_rabbit(white_rabbit, 2, 0, "follow_the_light");
			white_rabbit.move_white_rabbit(white_rabbit, 3, 0, "follow_the_light");
			white_rabbit.move_white_rabbit(white_rabbit, 4, 0, "follow_the_light");

			var _now_wait = Date.now();
			var _wait_voice_interval_ms = 20000;
			if (!white_rabbit.dock._waiting_voice_ts ||
					(_now_wait - white_rabbit.dock._waiting_voice_ts) >= _wait_voice_interval_ms) {
				white_rabbit.dock._waiting_voice_ts = _now_wait;
				if (white_rabbit.voice) white_rabbit.voice.say('Still searching for the light.');
			}

			// The moment the light returns, proceed to docking.
			if (white_rabbit.irlock.detected && white_rabbit.irlock.target) {
				white_rabbit.dock.dock_state = "docking_ground";
				white_rabbit.dock.docking_ground_started_at = Date.now();
				white_rabbit.dock.light_lost_since = null;
				white_rabbit.dock._waiting_voice_ts = null;
				if (white_rabbit.voice) white_rabbit.voice.say('Light found. Docking.');
			}
