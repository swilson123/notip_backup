...
var create_logs = function (white_rabbit) {
	...
	// Compass auto-calibration runs continuously — not just in the mission loop —
	// so the heading is already fine-tuned before a mission starts. It calibrates
	// only while driving straight forward (RC, undock, or mission); its own gates
	// handle everything else.
	setInterval(function () {
		if (white_rabbit.compass_calibration) white_rabbit.compass_calibration(white_rabbit);
	}, 1000);
