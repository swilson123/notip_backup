var follow_the_yellow_brick_road = function (white_rabbit) {
    ...
    if (white_rabbit.motor.steering_angle_deg) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            //Motor speed -- scaled by sidewalk_vision's edge-detection confidence (0-1), same
            //as the old per-edge-average formula this replaced: low confidence = slow down
            //to allow more time to (re)detect the path.
            var motor_speed_cmd = white_rabbit.realsense.path_detection.confidence * 100;
            ...
            // steering_angle_deg is relative (offset from wherever Noah currently faces);
            // carrot_target_hdg converts that to the same absolute 0-360 frame as hdg.
            var _carrot_target_hdg = ((white_rabbit.get_heading(white_rabbit) + white_rabbit.motor.steering_angle_deg) % 360 + 360) % 360;
            ...
            if (_log) _log.log(white_rabbit, {
                ...
                x_angle_deg: white_rabbit.realsense.path_detection.x_angle_deg,
            ...
            //Steering and RPM........................................
            if (Math.abs(white_rabbit.realsense.path_detection.x_angle_deg) > 20) {
                white_rabbit.motor.steering_angle_deg = white_rabbit.carrot(white_rabbit) / 6;
                ...
                white_rabbit.motor.steering_angle_deg = white_rabbit.carrot(white_rabbit) / 3;
                //front 2 wheel steering only.................................................
                white_rabbit.servo_send_command(white_rabbit, 11, white_rabbit.angle_to_pwm(white_rabbit.motor.steering_angle_deg).servo1, true);
                white_rabbit.servo_send_command(white_rabbit, 13, white_rabbit.angle_to_pwm(white_rabbit.motor.steering_angle_deg).servo2, true);
        ...
        if (white_rabbit.mission.package_delivered) {
            ...
            if (_current_wp && _current_wp.lat && _current_wp.lng) {
                ...
                if (white_rabbit.robot_data.robot_latitude && white_rabbit.robot_data.robot_longitude) {
                    ...
                    if (_distance_to_wp < 2.0) { // 2 meter threshold for waypoint reached
    ...
    //record sidewalk start lat/lng if not already set, so we can return to it after sidewalk-following is done
    if (!white_rabbit.mission.sidewalk_start_lat || !white_rabbit.mission.sidewalk_start_lng) {
        white_rabbit.mission.sidewalk_start_lat = white_rabbit.robot_data.robot_latitude;
        white_rabbit.mission.sidewalk_start_lng = white_rabbit.robot_data.robot_longitude;
    }
    else if (white_rabbit.mission.package_delivered) {

        //turn off follow the yellow brick road if we are back at the sidewalk start lat/lng
        if (white_rabbit.robot_data.robot_latitude && white_rabbit.robot_data.robot_longitude) {
            var _distance_to_wp = white_rabbit.gps_distance(white_rabbit.robot_data.robot_latitude, white_rabbit.robot_data.robot_longitude, white_rabbit.mission.sidewalk_start_lat, white_rabbit.mission.sidewalk_start_lng) * 1000;
            if (_distance_to_wp < 2.0) { // 2 meter threshold for waypoint reached
