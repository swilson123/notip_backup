var follow_the_yellow_brick_road = function (white_rabbit) {
    ...
    if (edge_left_x || edge_right_x) {
        ...
        // If one edge still missing after last-known fallback, synthesize it at center.
        // This keeps the dual centerline formula active and pulls Noah toward the visible edge.
        // Using 0 for x maximizes the correction: missing right → centerline pulls left, missing left → pulls right.
        var _synth_y = ((white_rabbit.realsense.vision_full && white_rabbit.realsense.vision_full.edge_lookahead_m) || 0.6096);
        if (!edge_right_x) { edge_right_x =  0.1; edge_right_y = _synth_y; }
        if (!edge_left_x)  { edge_left_x  = -0.1; edge_left_y  = _synth_y; }
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            if(white_rabbit.motor.steering_angle_deg > 20) white_rabbit.motor.steering_angle_deg = 20;
            if(white_rabbit.motor.steering_angle_deg < -20) white_rabbit.motor.steering_angle_deg = -20;
