var follow_the_yellow_brick_road = function (white_rabbit) {
    ...
    if (edge_left_x || edge_right_x) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            // Progressive tune for centerline formula (5-12° raw range).
            // < 1.5°: suppress jitter on straights. 1.5-6°: scale up to full. > 6°: amplify hard corrections.
            var abs_angle = Math.abs(white_rabbit.motor.steering_angle_deg);
            var steering_tune;
            if      (abs_angle < 1.5) steering_tune = 0.50;
            else if (abs_angle < 3.5) steering_tune = 0.80;
            else if (abs_angle < 6)   steering_tune = 1.00;
            else if (abs_angle < 9)   steering_tune = 1.30;
            else                      steering_tune = 1.50;
            ...
            // This is the smoothing mechanism — a hard correction (e.g. 8° raw) ramps up
            // over 2-4 ticks rather than slamming instantly, while still delivering the
            // full correction within ~1 second. Do NOT add EMA on top: it slows the
            // correction buildup and causes Noah to hug the edge before recovering.
