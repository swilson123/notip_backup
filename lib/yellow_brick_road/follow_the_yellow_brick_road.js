var follow_the_yellow_brick_road = function (white_rabbit) {
    ...
    // Imagine a line and I steer towards that line but overshoot it...then I turn back towards that line and overshoot it again....each wave causing more and more correctional errors especially around turns. This is what Noah is doing.
    // When you drive a car you don't spin the steering wheel to the angle of the turn in one tik...You do it over time.
    //white_rabbit.motor.steering_angle_deg = white_rabbit.motor.steering_angle_deg  / 2;


    if (white_rabbit.motor.carrot_has_edge) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            // Noah's actual heading vs. the heading the carrot wants him pointed at, side by
            // side so it's directly visible whether one is tracking the other tick to tick.
            // carrot_heading_deg is relative (offset from wherever Noah currently faces);
            // carrot_target_hdg converts that to the same absolute 0-360 frame as hdg.
            var _carrot_target_hdg = ((white_rabbit.get_heading(white_rabbit) + white_rabbit.motor.carrot_heading_deg) % 360 + 360) % 360;

            // Remembered so the NO EDGES DETECTED fallback below can try to yaw back to
            // where the sidewalk actually was, instead of jumping straight to the (possibly
            // very different, especially mid-curve) GPS bearing to the next waypoint.
            white_rabbit.motor.last_known_carrot_target_hdg = _carrot_target_hdg;
            white_rabbit.motor.last_known_carrot_target_hdg_ts = Date.now();
            ...
            if (_log) _log.log(white_rabbit, {
                ...
                carrot_heading: Math.round(white_rabbit.motor.carrot_heading_deg * 10) / 10,
                carrot_target_hdg: Math.round(_carrot_target_hdg * 10) / 10,
                ...
                carrot_bias_m: Math.round((white_rabbit.motor.carrot_bias_m || 0) * 1000) / 1000,
    ...
    if (!white_rabbit.motor.moving) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            // Prefer yawing back to the last heading the carrot actually had us pointed at
            // (recent only -- a stale value from minutes ago is more likely wrong than
            // helpful) over the GPS bearing to the next waypoint. That waypoint bearing can
            // be a completely different direction than the sidewalk was actually running,
            // especially mid-curve -- jumping straight to it throws away the one signal that
            // knows which way the sidewalk was pointing right before the camera lost it.
            var _last_known_ttl_ms = (white_rabbit.realsense.vision_full && typeof white_rabbit.realsense.vision_full.carrot_last_known_hdg_ttl_ms === 'number')
                ? white_rabbit.realsense.vision_full.carrot_last_known_hdg_ttl_ms : 3000;
            var _has_recent_carrot_hdg = typeof white_rabbit.motor.last_known_carrot_target_hdg === 'number' &&
                typeof white_rabbit.motor.last_known_carrot_target_hdg_ts === 'number' &&
                (Date.now() - white_rabbit.motor.last_known_carrot_target_hdg_ts) < _last_known_ttl_ms;

            if (_has_recent_carrot_hdg && typeof white_rabbit.get_heading(white_rabbit) === 'number') {
                var _carrot_yaw_error = ((white_rabbit.motor.last_known_carrot_target_hdg - white_rabbit.get_heading(white_rabbit) + 540) % 360) - 180;
                if (Math.abs(_carrot_yaw_error) > 5) {
                    white_rabbit.yaw_white_rabbit(white_rabbit, _carrot_yaw_error, 20);
                }
            } else {
                // Last-known carrot heading is stale (or never set) -- fall back to yawing
                // Noah towards the next waypoint to scan for edges again.
                var _wp = null;
