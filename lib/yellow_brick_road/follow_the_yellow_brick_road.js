    ...
    if (white_rabbit.motor.carrot_has_edge || 1 == 1) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            if (_log) _log.log(white_rabbit, {
                ...
                // Full near-to-far per-band trace, not just the single closest band picked
                // above -- el1 is nearest, el6 (if present) is farthest. Read x_m across the
                // trace to see a curve starting before the "closest" pick alone would show it.
                el_bands: (white_rabbit.realsense.path_detection.edge_left_bands || []).map(b => b.x_m),
                er_bands: (white_rabbit.realsense.path_detection.edge_right_bands || []).map(b => b.x_m),
