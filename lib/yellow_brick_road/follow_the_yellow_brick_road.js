var follow_the_yellow_brick_road = function (white_rabbit) {
    ...
    if (edge_left_x || edge_right_x) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            // Prevents reacquisition spikes and S-bend overshoots. 4°/tick keeps Noah
            // from accumulating heading error faster than the camera can observe and correct.
    ...
    if (!white_rabbit.motor.moving) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            // Guard: only yaw if edges were previously seen. At fresh mission start
            // last_known is null, so no yaw fires — Noah holds position while the camera
            // acquires the path. After edges are seen then lost (corner recovery), last_known
            // is set and yaw fires normally to sweep Noah back toward the next waypoint.
