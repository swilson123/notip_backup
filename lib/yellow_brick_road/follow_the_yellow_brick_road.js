var follow_the_yellow_brick_road = function (white_rabbit) {
    ...
    //Steering comes straight from realsense rather than carrot.js's local math. Live edge
    //guidance (x_angle_deg) is primary; when Python itself says the live camera frame can't
    //be trusted (edge_guidance_valid false — e.g. camera panned off the sidewalk mid-turn),
    //fall back to map_angle_deg, the persistent heading-anchored path_map's projection of
    //the sidewalk center at the same lookahead distance, re-expressed in Noah's current
    //heading. Both use the same sign convention (>0 = target to the RIGHT), so the same
    //correction_direction flip (calc_steering_and_rpm treats steering_angle_deg > 0 as turn
    //LEFT) applies to whichever source is chosen.
    ...
    white_rabbit.motor.steering_angle_deg =
    ...
    if (white_rabbit.motor.carrot_has_edge || 1 == 1) {
        ...
        if (!white_rabbit.mission.pause_mission) {
            ...
            if (_log) _log.log(white_rabbit, {
                ...
                steer_src: white_rabbit.realsense.path_detection.edge_guidance_valid ? 'live' : 'map',
                map_angle: white_rabbit.realsense.path_detection.map_angle_deg,
                map_conf: white_rabbit.realsense.path_detection.map_confidence,
