// Steering tuning constants -- these used to be realsense_vision.* setup.json params.
// That config section was trimmed down to only what the camera connection itself
// needs; these values are the ones that were actually configured before the trim, so
// current steering behavior is unchanged. See CLAUDE.md's realsense_vision note.
var CORRECTION_DIRECTION = 1;
var EDGE_STEER_DEADBAND_DEG = 2;
var SIDEWALK_STEER_MAX_DEG = 10;
var SIDEWALK_STEER_INPUT_MAX_DEG = 20;
var STEERING_TIME_CONSTANT_S = 0.6;
...
var carrot = function (white_rabbit) {
    ...
    if (Math.abs(_steer_target_deg) < EDGE_STEER_DEADBAND_DEG) {
        _steer_target_deg = 0;
    }
    ...
    var _steer_max_deg = SIDEWALK_STEER_MAX_DEG;
    var _steer_input_max_deg = SIDEWALK_STEER_INPUT_MAX_DEG;
    ...
    if (white_rabbit.motor.steering_smooth_ts) {
        ...
        var _steer_tau_s = STEERING_TIME_CONSTANT_S;
