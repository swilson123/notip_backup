...
var notip_init = function (config) {
    const white_rabbit = {
        ...
        nav_tuning: {
            ...
            // Distance (m) to drive past the bottom of the ramp before fixing the
            // undock return point and raising the arm. Measured by wheel odometry.
            post_ramp_drive_m: config.nav_tuning && typeof config.nav_tuning.post_ramp_drive_m === 'number' ? config.nav_tuning.post_ramp_drive_m : 2.0,
            // Progress monitor: correct a gross heading fault when Noah drives away from the waypoint.
            nav_progress_monitor_enabled: !(config.nav_tuning && config.nav_tuning.nav_progress_monitor_enabled === false)
        },
        imu: {
            enabled: !(config.imu && config.imu.enabled === false),
            compass_offset_deg: config.imu && typeof config.imu.compass_offset_deg === 'number' ? config.imu.compass_offset_deg : 0,
            pixhawk_offset_deg: config.imu && typeof config.imu.pixhawk_offset_deg === 'number' ? config.imu.pixhawk_offset_deg : 0,
            // Off by default — see compass_calibration.js. The EKF heading is used raw.
            calibrate_pixhawk_heading: !!(config.imu && config.imu.calibrate_pixhawk_heading === true),
        ...
        waypoint_progress_monitor: require("./navigation/waypoint_progress_monitor"),
    ...
    // Return the instance so the entry point (server.js) can shut it down cleanly.
    return white_rabbit;
