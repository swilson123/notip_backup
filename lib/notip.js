...
var notip_init = function (config) {
    const white_rabbit = {
        ...
        imu: {
            ...
            // Which compass driver to use, both read over the Pi's I2C bus 1 (SDA/SCL):
            //   "witmotion" → connect_to_witmotion.js (WitMotion HWT906)
            //   "bno055"    → connect_to_imu.js (original Bosch BNO055 — rollback path)
            // Defaults to "bno055" when the key is absent, so old setups behave exactly as before.
            compass_type: (config.imu && config.imu.compass_type === 'witmotion') ? 'witmotion' : 'bno055',
        ...
        mission: {
            ...
            // GPS position recorded the moment sidewalk_follow_active turns ON. On the
            // return trip, coming back within 1m of this exact spot also closes the gate
            // (independent of the waypoint-based close above). See run_mission.js.
            sidewalk_gate_open_lat: null,
            sidewalk_gate_open_lng: null,
            // current_mission_seq recorded at the gate waypoint just before sidewalk-following
            // turns ON. Restored verbatim when the gate closes so GPS nav resumes from this
            // known-good waypoint rather than wherever the camera-steered stretch left
            // current_mission_seq (it only advances on a 1m proximity hit, which a sidewalk
            // edge path doesn't guarantee for every mapped waypoint). See run_mission.js.
            sidewalk_gate_seq: null,
