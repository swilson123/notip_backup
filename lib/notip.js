...
var notip_init = function (config) {
    const white_rabbit = {
        ...
        nav_tuning: {
            ...
            // Noah's dock arrival radius (and the overshoot-capture debounce) can latch
            // "reached" while still up to overshoot_capture_m short of the true undock
            // position — GPS bearing gets noisy this close in. Once dock arrival first
            // latches, run_mission pushes the target this much further along the same
            // approach bearing and drives that extra distance for real before switching
            // to heading alignment, instead of stopping the instant the debounce fires.
            dock_arrival_overrun_m: config.nav_tuning && typeof config.nav_tuning.dock_arrival_overrun_m === 'number' ? config.nav_tuning.dock_arrival_overrun_m : 1.0,
        ...
        mission: {
            ...
            // Set once the dock-arrival overrun target has been projected for this
            // return leg, so run_mission only extends the target once. See run_mission.js.
            dock_overrun_consumed: false,
