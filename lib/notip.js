    ...
    const white_rabbit = {
        ...
        realsense:{
            ...
            path_detection: {
                ...
                // Fallback steering angle projected from path_map (persistent, heading-anchored
                // sidewalk history) at the same lookahead distance as x_angle_deg. Used when
                // edge_guidance_valid is false — e.g. the camera has panned off the sidewalk
                // mid-turn and the live frame has nothing to steer off.
                map_angle_deg: null,
                map_confidence: 0,
                source: null
            },
            ...
            vision: {
                ...
                // Steering-output smoothing: one physical knob (seconds to settle onto a
                // new target angle) instead of separate gain + max-degrees-per-tick
                // constants, since "per tick" is meaningless once fps throttles under CPU
                // load. See follow_the_yellow_brick_road.js.
                steering_time_constant_s: config.realsense_vision && typeof config.realsense_vision.steering_time_constant_s === 'number' ? config.realsense_vision.steering_time_constant_s : 0.6,
                edge_steer_deadband_deg: config.realsense_vision && typeof config.realsense_vision.edge_steer_deadband_deg === 'number' ? config.realsense_vision.edge_steer_deadband_deg : 2,
                ...
                // Raw camera angle treated as "full deflection" -- carrot.js linearly scales
                // the clamped [-this, +this] range onto [-sidewalk_steer_max_deg, +sidewalk_steer_max_deg],
                // the same clamp-then-rescale pattern pwm_to_angle.js uses for RC stick input.
                sidewalk_steer_input_max_deg: config.realsense_vision && typeof config.realsense_vision.sidewalk_steer_input_max_deg === 'number' ? config.realsense_vision.sidewalk_steer_input_max_deg : 45,
        ...
        edge_trail: require("./realsense/edge_trail"),
        ...
        carrot_data_filter: require('./yellow_brick_road/carrot_data_filter.js'),
