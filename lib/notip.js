...
var notip_init = function (config) {
    const white_rabbit = {
        ...
        nav_tuning: {
            ...
            delivery_timeout_ms: config.nav_tuning && typeof config.nav_tuning.delivery_timeout_ms === 'number' ? config.nav_tuning.delivery_timeout_ms : 45000,
            ...
            arrival_radius_dock_m: config.nav_tuning && typeof config.nav_tuning.arrival_radius_dock_m === 'number' ? config.nav_tuning.arrival_radius_dock_m : 0.1,
            arrival_radius_delivery_m: config.nav_tuning && typeof config.nav_tuning.arrival_radius_delivery_m === 'number' ? config.nav_tuning.arrival_radius_delivery_m : 0.1,
            arrival_radius_waypoint_m: config.nav_tuning && typeof config.nav_tuning.arrival_radius_waypoint_m === 'number' ? config.nav_tuning.arrival_radius_waypoint_m : 0.5,
            overshoot_capture_m: config.nav_tuning && typeof config.nav_tuning.overshoot_capture_m === 'number' ? config.nav_tuning.overshoot_capture_m : 1.0,
            // Bypasses the gate-waypoint requirement in follow_sidewalk_enabled() so sidewalk
            // steering is on from the start of the mission — useful for testing without needing
            // to reach a gate waypoint first. Defaults false so old setups behave exactly as before.
            sidewalk_gate_always_open: !!(config.nav_tuning && config.nav_tuning.sidewalk_gate_always_open === true),
            // System-wide steering bias correction (degrees), applied once in angle_to_pwm
            // to every steering angle before it's mapped to servo PWM -- sidewalk-following,
            // docking/undocking, manual RC, and voice nudges all funnel through angle_to_pwm,
            // so a single mechanical pull (e.g. Noah hugging the left side of the sidewalk AND
            // the dock ramps) is corrected in one place instead of duplicated per caller.
            steering_trim_deg: config.nav_tuning && typeof config.nav_tuning.steering_trim_deg === 'number' ? config.nav_tuning.steering_trim_deg : 0,
            // Width (PWM us) of the dead zone around chan4_raw's 1500 center that radio_commands.js
            // requires before treating the stick as an intentional yaw (spin-in-place) command.
            // chan3 (throttle) and chan4 (yaw) sit on the same physical gimbal, so pushing straight
            // forward/back on throttle can bleed enough into chan4 to cross a narrow band and fire
            // yaw_white_rabbit when the operator only meant to move forward/reverse. Wider than the
            // old hardcoded +/-50 to absorb that cross-axis drift before it's read as real yaw intent.
            ...
            // Dock policy: when true, Noah never auto-docks on the return. After GPS +
            // heading alignment brings the IRLock beacon into view he parks in
            // await_dock_command and waits for an explicit trigger — the RC dock command
            // OR the hook limit switch — before running follow_the_light. run_mission
            // seeds the mutable white_rabbit.dock.manual_dock_required from this policy
            // when the dock-return begins (reset_white_rabbit clears the runtime flag, so
            // the lasting policy lives here on nav_tuning). Defaults false = auto-dock,
            // preserving the old behavior when the key is absent.
            manual_dock_required: !!(config.nav_tuning && config.nav_tuning.manual_dock_required === true)
        },
        imu: {
            // When true, get_heading/get_pitch/get_roll use the IMU; when false they use the Pixhawk.
            enable_imu: !!(config.imu && config.imu.enable_imu === true),
            // Which compass driver to use:
            //   "witmotion"        → connect_to_witmotion.js (WitMotion HWT906, I2C bus 1 SDA/SCL)
            //   "witmotion_hwt905" → connect_to_witmotion_hwt905.js (WitMotion HWT905-TTL, UART TX/RX)
            //   "bno055"           → connect_to_imu.js (original Bosch BNO055, I2C — rollback path)
            // Defaults to "bno055" when the key is absent, so old setups behave exactly as before.
            compass_type: (config.imu && (config.imu.compass_type === 'witmotion' || config.imu.compass_type === 'witmotion_hwt905'))
                ? config.imu.compass_type
                : 'bno055',
            ...
            // HWT906 orientation knobs (X=fwd/back, Y=left/right mount). Only used when compass_type==="witmotion".
            witmotion: {
                i2c_address: config.imu && config.imu.witmotion && typeof config.imu.witmotion.i2c_address === 'number' ? config.imu.witmotion.i2c_address : 0x50,
                heading_invert: !(config.imu && config.imu.witmotion && config.imu.witmotion.heading_invert === false), // default true: CCW yaw → CW compass
                swap_roll_pitch: !!(config.imu && config.imu.witmotion && config.imu.witmotion.swap_roll_pitch === true),
                invert_roll: !!(config.imu && config.imu.witmotion && config.imu.witmotion.invert_roll === true),
                invert_pitch: !!(config.imu && config.imu.witmotion && config.imu.witmotion.invert_pitch === true)
            },
            // HWT905-TTL orientation + serial knobs. Only used when compass_type==="witmotion_hwt905".
            witmotion_hwt905: {
                port_path: config.witmotion_hwt905_comName,
                baud_rate: config.imu && config.imu.witmotion_hwt905 && typeof config.imu.witmotion_hwt905.baud_rate === 'number' ? config.imu.witmotion_hwt905.baud_rate : 9600,
                heading_invert: !(config.imu && config.imu.witmotion_hwt905 && config.imu.witmotion_hwt905.heading_invert === false), // default true: CCW yaw → CW compass
                swap_roll_pitch: !!(config.imu && config.imu.witmotion_hwt905 && config.imu.witmotion_hwt905.swap_roll_pitch === true),
                invert_roll: !!(config.imu && config.imu.witmotion_hwt905 && config.imu.witmotion_hwt905.invert_roll === true),
                invert_pitch: !!(config.imu && config.imu.witmotion_hwt905 && config.imu.witmotion_hwt905.invert_pitch === true),
                // Z-axis (yaw) algorithm re-asserted on every connect — see assert_algorithm() in
                // connect_to_witmotion_hwt905.js. "9-axis" (default) is magnetometer-fused and absolute
                // across power cycles; "6-axis" is gyro-only and resets to 0 every boot (WitMotion's own
                // documented tradeoff for dodging magnetic interference near the motors).
                algorithm: (config.imu && config.imu.witmotion_hwt905 && config.imu.witmotion_hwt905.algorithm === '6-axis') ? '6-axis' : '9-axis',
                enforce_algorithm_on_connect: !(config.imu && config.imu.witmotion_hwt905 && config.imu.witmotion_hwt905.enforce_algorithm_on_connect === false),
                serial: null
            }
        },
        zling: {
        ...
        mission: {
            ...
            sidewalk_start_lat: null,
            sidewalk_start_lng: null,
        ...
        zones: [
            { zone: 1, light: "red", min_angle: 30, max_angle: 60, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 2, light: "red", min_angle: 60, max_angle: 90, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 3, light: "red", min_angle: 90, max_angle: 120, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 4, light: "red", min_angle: 120, max_angle: 150, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 5, light: "red", min_angle: 150, max_angle: 180, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 6, light: "red", min_angle: 180, max_angle: 210, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 7, light: "red", min_angle: 210, max_angle: 240, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 8, light: "red", min_angle: 240, max_angle: 270, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 9, light: "red", min_angle: 270, max_angle: 300, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 10, light: "red", min_angle: 300, max_angle: 330, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 11, light: "red", min_angle: 330, max_angle: 360, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
            { zone: 12, light: "red", min_angle: 0, max_angle: 30, min_distance_mm: 100, max_distance_mm: 1000, timestamp: null, distance_mm: null, angle: null },
        ],
        mav_version: 1,
        waveshare: {
            baudrate: 115200,
        ...
        arduino: {
            ...
            parser: null,
            received_data: {},
            // Previous hook_switch_state, for rising-edge detection in
            // arduino_message_handler. hook_switch_state is a held level (true the
            // whole time the switch is closed), so a press is the false->true edge.
            hook_switch_prev: false,
            belt_extend_switch_state: 1

        },
        ldr: {
            //Model STL-19P — fixed protocol baud rate, not configurable per install
            baudrate: 230400,
            port_path: config.ldr_comName,
            connected: false,
            serial: null,
            received_data: {},

            // Config — mirrors ldr section of setup.json. Mounted vertically on the front
            // of Noah: 0 deg points straight down, 180 deg straight up. Angles 0-180 sweep
            // one side of the rover, 180-360 sweep the other — side_invert flips which is
            // which if the mount/spin direction wires up backwards on hardware.
            enabled: config.ldr ? config.ldr.enabled !== false : true,
            mount_height_mm: config.ldr && typeof config.ldr.mount_height_mm === 'number' ? config.ldr.mount_height_mm : 609.6, // 24 in
            edge_delta_threshold_mm: config.ldr && typeof config.ldr.edge_delta_threshold_mm === 'number' ? config.ldr.edge_delta_threshold_mm : 100,
            edge_min_theta_deg: config.ldr && typeof config.ldr.edge_min_theta_deg === 'number' ? config.ldr.edge_min_theta_deg : 5,
            edge_max_theta_deg: config.ldr && typeof config.ldr.edge_max_theta_deg === 'number' ? config.ldr.edge_max_theta_deg : 85,
            side_invert: config.ldr ? config.ldr.side_invert === true : false,

            // Display — launches a browser window plotting the live scan when enabled.
            display_enabled: config.ldr ? config.ldr.display_enabled === true : false,
            display_port: config.ldr && typeof config.ldr.display_port === 'number' ? config.ldr.display_port : 8090,
            display_browser_path: config.ldr && typeof config.ldr.display_browser_path === 'string' ? config.ldr.display_browser_path : 'chromium-browser',
            display_broadcast_interval_ms: config.ldr && typeof config.ldr.display_broadcast_interval_ms === 'number' ? config.ldr.display_broadcast_interval_ms : 100,
            display_server: null,
            display_process: null,
            display_last_payload: null,
            display_last_broadcast_ms: null,

            // State — one slot per whole degree (0-359) of the vertical scan plane,
            // refreshed as packets arrive; ldr_edge_data reads this each call.
            scan: new Array(360).fill(null),
            edge_left_distance_mm: null,
            edge_right_distance_mm: null
        ...
        realsense: {
            ...
            // Raw, uncurated realsense_vision config — forwarded wholesale to the Python
            // subprocess so EVERY setup.json key reaches the detector. The curated `vision`
            // object below stays for Node-side consumers (it is intentionally a subset).
            vision_full: config.realsense_vision || {},
            vision: {
                ...
                script_path: config.realsense_vision && config.realsense_vision.script_path ? config.realsense_vision.script_path : './lib/sidewalk_vision/sidewalk_vision.py',
                screen_width: config.realsense_vision && typeof config.realsense_vision.screen_width === 'number' ? config.realsense_vision.screen_width : 1280,
                screen_height: config.realsense_vision && typeof config.realsense_vision.screen_height === 'number' ? config.realsense_vision.screen_height : 720,
                ...
                camera_mount_pitch_deg: config.realsense_vision && typeof config.realsense_vision.camera_mount_pitch_deg === 'number' ? config.realsense_vision.camera_mount_pitch_deg : 0
            }
        ...
        irlock: {
            ...
            follow_config: {
                ...
                post_ramp_drive_distance_m: config.irlock && typeof config.irlock.post_ramp_drive_distance_m === 'number' ? config.irlock.post_ramp_drive_distance_m : 2.0,
                ...
                pre_search_drive_m: config.irlock && typeof config.irlock.pre_search_drive_m === 'number' ? config.irlock.pre_search_drive_m : 0.4572,
                steer_smoothing_alpha: config.irlock && typeof config.irlock.steer_smoothing_alpha === 'number' ? config.irlock.steer_smoothing_alpha : 0.4,
                steer_deadband_deg: config.irlock && typeof config.irlock.steer_deadband_deg === 'number' ? config.irlock.steer_deadband_deg : 2.0,
                search_timeout_ms: config.irlock && typeof config.irlock.search_timeout_ms === 'number' ? config.irlock.search_timeout_ms : 20000,
                search_say_interval_ms: config.irlock && typeof config.irlock.search_say_interval_ms === 'number' ? config.irlock.search_say_interval_ms : 10000
            }
        },
        down_the_rabbit_hole: require("./dock/down_the_rabbit_hole"),
        connect_to_irlock: require("./irock/connect_to_irlock"),
        irlock_message_handler: require("./irock/irlock_message_handler"),
        follow_the_light: require("./dock/follow_the_light"),
        ...
        connect_to_witmotion: require('./imu/connect_to_witmotion.js'),
        ...
        voice_config: {
            ...
            low_battery_repeat_ms: config.voice && typeof config.voice.low_battery_repeat_ms === 'number' ? config.voice.low_battery_repeat_ms : 30000
        },
        compass_calibration: require('./imu/compass_calibration.js'),
        follow_the_yellow_brick_road: require('./yellow_brick_road/follow_the_yellow_brick_road.js'),
        carrot: require('./yellow_brick_road/carrot.js'),
        connect_to_ldr: require('./ldr/connect_to_ldr.js'),
        ldr_message_handler: require('./ldr/ldr_message_handler.js'),
        calc_ldr_crc8: require('./ldr/ldr_crc8.js'),
        ldr_edge_data: require('./ldr/ldr_edge_data.js'),
        connect_to_ldr_display: require('./ldr/connect_to_ldr_display.js'),
        connect_to_witmotion_hwt905: require('./imu/connect_to_witmotion_hwt905.js'),
        start_mission: require('./mission/start_mission.js'),
        //WitMotion MJRTK-UM982 — dual-antenna RTK GNSS. Mounted as connect_to_witmotion_mjrtk
        //because white_rabbit.connect_to_witmotion is already the HWT906 I2C compass (line 631);
        //reusing that name silently replaced the compass driver.
        witmotion_message_handler: require('./witmotion/witmotion_message_handler.js'),
        connect_to_witmotion_mjrtk: require('./witmotion/connect_to_witmotion.js'),
        create_witmotion_message: require('./witmotion/create_witmotion_message.js'),
        witmotion: {
            //UM982 ships at 115200. connect_to_witmotion.js warns after 5s of unparseable bytes,
            //which is what a baud mismatch looks like from the outside.
            baudrate: typeof config.witmotion_mjrtk_baudrate === 'number' ? config.witmotion_mjrtk_baudrate : 115200,
            port_path: config.witmotion_mjrtk_comName,
            connected: false,
            serial: null,
            //No stream parser: the device interleaves ASCII sentences with binary 0x55 IMU frames
            //on one wire, so a line parser would shred the binary. connect_to_witmotion.js frames it.
            frames_ok: 0,
            frames_bad: 0,
            last_message_ts: 0,
            last_log_ts: 0,
            received_data: {
                gps: {
                    ...
                    fix_quality: null, fix_type: 'no_fix', rtk: false,
                    satellites: null, hdop: null,
                    speed_mps: null, course_deg: null,
                    utc: null, date: null, valid: false,
                    accuracy: { lat_std_m: null, lon_std_m: null, alt_std_m: null }
                },
                //Heading and pitch come from the antenna baseline — no magnetometer, so the
                //drivetrain cannot pull them. Roll is not observable from two antennas.
                heading_deg: null, heading_std_deg: null,
                pitch_deg: null, pitch_std_deg: null,
                baseline_m: null, heading_solution: null, heading_type: null,
                //Populated only if this board also streams a WitMotion IMU (0x55 packets).
                imu: {
                    present: false,
                    roll_deg: null, pitch_deg: null, yaw_deg: null,
                    accel: { x: 0, y: 0, z: 0 },
                    gyro: { x: 0, y: 0, z: 0 },
                    temperature_c: null
                },
