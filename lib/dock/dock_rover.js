var dock_rover = function (rover) {
	...
	if (rover.robot_data.is_armed) {
		...
		if (!rover.dock.dock_state) {
			...
			rover.dock.undock_heading = rover.robot_data.VFR_HUD ? rover.robot_data.VFR_HUD.heading : null;

			rover.dock.ramp_started_at = null;
		}
		else if (rover.dock.dock_state === "docking_ground") {
			// Back rover up to start climbing the ramp in reverse.
			...
			var pitch_delta = rover.robot_data.ATTITUDE.pitch - rover.dock.undock_pitch;
			...
			var ramp_pitch_delta = rover.robot_data.ATTITUDE.pitch - rover.dock.undock_pitch;
			...
			rover.dock.dock_heading = rover.robot_data.VFR_HUD ? rover.robot_data.VFR_HUD.heading : null;
