var dock_white_rabbit = function (white_rabbit) {
	...
	if (white_rabbit.robot_data.is_armed) {
		// Wait for a live Pixhawk ATTITUDE before driving — undock_pitch is captured
		// once at dock start and a stale reading would break ramp detection. Don't
		// require the external IMU: it may be disabled (imu.enabled=false), and all
		// pitch reads go through get_pitch, which falls back to the Pixhawk ATTITUDE.
