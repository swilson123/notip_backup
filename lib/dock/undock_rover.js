var undock_rover = function (rover) {
    if (rover.robot_data.is_armed) {
        ...
        if (!rover.imu_data.connected || !rover.robot_data.ATTITUDE.pitch) {
        ...
        // If the rover never detects a ramp (parked on flat ground, missing
        ...
        if (!rover.dock.dock_state) {
            ...
            rover.dock.docked_state_started_at = Date.now();
            ...
            else if (Date.now() - rover.dock.docked_state_started_at >= docked_state_timeout_ms) {
            ...
            // If a full mission was requested via the RC undock switch, start it now.
            if (rover.dock.start_mission_after_undock && !rover.robot_data.mission_mode) {
