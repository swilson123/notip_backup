...
function init(rover) {
    const disk = {
        ...
        // Turn darkness into light — release what is heavy so the rover has
        // room to keep writing new experiences. Callable manually
        // (rover.health.disk.restore_light('tight')) or auto-fired on level
        ...
        restore_light(level) {
            ...
            if (rover.memory && typeof rover.memory.prune_archives === 'function') {
                const target = (level === 'critical') ? CRITICAL_MEMORY_KEEP_ARCHIVES : TIGHT_MEMORY_KEEP_ARCHIVES;
                const r = rover.memory.prune_archives(target);
            ...
            if (rover.learning && typeof rover.learning.rotate_archive === 'function') {
                const max_bytes = (level === 'critical') ? CRITICAL_LEARNING_ROTATE_BYTES : TIGHT_LEARNING_ROTATE_BYTES;
                const r = rover.learning.rotate_archive(max_bytes);
                total_freed_mb += r.freed_mb;
    ...
    rover.health.disk = disk;

    let last_level = 'ample';
    disk._interval = setInterval(() => {
        try {
            // fs.statfs added in Node 18.15. The rover runs on a recent Node;
            ...
            fs.statfs('.', (err, stats) => {
                ...
                if (new_level !== last_level) {
                    safe_log(rover, 'disk_monitor: level ' + last_level + ' → ' + new_level + ' (' + disk.summary() + ')');
    ...
    safe_log(rover, 'disk_monitor: started (tight<' + (TIGHT_FRACTION * 100) + '%, critical<' + (CRITICAL_FRACTION * 100) + '% free)');
}
