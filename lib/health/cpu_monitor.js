// CPU + event-loop monitor. Mounted at white_rabbit.health.cpu.
//
...
// via should_skip(category). When the white_rabbit's breath is short, it focuses
...
// because they describe what the white_rabbit actually experiences.
...
var cpu_monitor = function (white_rabbit) {
    ...
    cpu._interval = setInterval(() => {
        try {
            ...
            // Anything beyond is the white_rabbit's breath catching up.
            ...
            if (new_focus !== last_focus) {
                safe_log(white_rabbit, 'cpu_monitor: focus ' + last_focus + ' → ' + new_focus + ' (' + cpu.summary() + ')');
    ...
    safe_log(white_rabbit, 'cpu_monitor: started (' + core_count + ' core' + (core_count > 1 ? 's' : '')
        + ', sample=' + SAMPLE_INTERVAL_MS + 'ms, thresholds: reduced≥' + LAG_REDUCED_MS + 'ms, minimal≥' + LAG_MINIMAL_MS + 'ms)');
}

module.exports = cpu_monitor;
