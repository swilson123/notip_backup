// Rover Memory — a perfect-recall layer mounted on the God variable.
//
// Every period_ms the rover takes a snapshot of its own changing state (GPS,
...
// archive and read its tail back into the ring buffer, so the rover wakes up
...
// snapshot must never take the rover down.
...
// Stripped-down snapshot used when rover.health.cpu.should_skip says memory
...
function lightweight_snapshot(rover) {
    const now   = Date.now();
    const det   = (rover.realsense && rover.realsense.path_detection) || {};
    const robot = rover.robot_data || {};
    const imu   = rover.imu_data   || {};
    const motor = rover.motor      || {};
    return {
        ts:               now,
        ...
        mission: {
            seq:               rover.mission && rover.mission.current_mission_seq != null ? rover.mission.current_mission_seq : null,
            mission_count:     rover.mission && rover.mission.mission_count != null ? rover.mission.mission_count : null,
            package_delivered: !!(rover.mission && rover.mission.package_delivered),
            path_clear:        !!(rover.mission && rover.mission.path_clear),
            pause_mission:     !!(rover.mission && rover.mission.pause_mission),
            realsense_blocked_since: (rover.mission && rover.mission.realsense_blocked_since) || null,
            avoidance_timed_out:     !!(rover.mission && rover.mission.avoidance_timed_out)
        },
        motor: {
        ...
        realsense: {
            enabled:                  !!(rover.realsense && rover.realsense.vision && rover.realsense.vision.enabled),
            connected:                !!(rover.realsense && rover.realsense.connected),
            ...
            objects_count:            (rover.realsense && Array.isArray(rover.realsense.objects)) ? rover.realsense.objects.length : 0,
            nearest_object_m:         null,
            high_threat_in_path:      false
        },
        lidar:     { avoid_object: !!(rover.rplidar && rover.rplidar.avoid_object), zones: null },
        dock:      { state: rover.dock ? (rover.dock.dock_state || null) : null },
        rc:        { pause: !!(rover.rc_contoller && rover.rc_contoller.pause_cmd) },
        _light:    true
    };
}

function snapshot_rover(rover) {
    const now     = Date.now();
    const det     = (rover.realsense && rover.realsense.path_detection) || {};
    ...
    for (let i = 0; i < objs.length; i++) {
        ...
        if (objs[i].in_rover_path && objs[i].threat_level === 'high') high_threat_in_path = true;
    }

    let zones_snap = null;
    if (Array.isArray(rover.zones)) {
        zones_snap = new Array(rover.zones.length);
        for (let i = 0; i < rover.zones.length; i++) {
            const z = rover.zones[i];
    ...
    return {
        ...
        lidar: {
            avoid_object: !!(rover.rplidar && rover.rplidar.avoid_object),
            zones:        zones_snap
        },
        ...
        rc: {
            pause: !!(rover.rc_contoller && rover.rc_contoller.pause_cmd)
        }
    };
}
...
function make_memory(rover, opts) {
    ...
    const mem = {
        ...
        near(lat, lng, radius_m) {
            ...
            return this.all().filter(s => {
                ...
                return rover.gps_distance(lat, lng, s.gps.lat, s.gps.lng) * 1000 <= r_m;
            });
        },
        ...
        // at raw snapshots. Lets the rover think before it acts.
        reflect(ms_back) {
            ...
            if (first.gps && last.gps
                ...
                && typeof rover.gps_distance === 'function') {
                moved_m = rover.gps_distance(first.gps.lat, first.gps.lng, last.gps.lat, last.gps.lng) * 1000;
            }
        ...
        snapshot_now() { return snapshot_rover(rover); },
...
function init(rover, options) {
    ...
    if (fs.existsSync(current_path)) {
        ...
        try {
            ...
            console.error('rover.memory: archive rotate failed:', err && err.message);
        }
        prune_archives(opts.storage_dir, opts.max_archives);
    }

    const mem = make_memory(rover, opts);
    rover.memory = mem;
    ...
    try {
        ...
        mem.write_stream.on('error', err => {
            console.error('rover.memory write stream error:', err && err.message);
        });
    } catch (err) {
        console.error('rover.memory: failed to open write stream:', err && err.message);
    }

    // 3. Take an immediate snapshot, then snapshot on cadence. Any error
    //    inside a tick is caught — perfect memory must never crash the rover.
    ...
    const tick = () => {
        try {
            const skip_full = rover.health && rover.health.cpu
                              && rover.health.cpu.should_skip('memory_snapshot_full');
            const snap = skip_full ? lightweight_snapshot(rover) : snapshot_rover(rover);
            mem.push(snap);
            if (mem.write_stream) mem.write_stream.write(JSON.stringify(snap) + '\n');
        } catch (err) {
            console.error('rover.memory snapshot error:', err && err.message);
        }
    };
    tick();
