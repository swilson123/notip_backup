...
const STORAGE_DIR  = path.join('logger', 'memory');
...
function make_memory(white_rabbit, period_ms, window_ms, storage_dir) {
...
var white_rabbit_memory = function (white_rabbit) {

    const cfg          = white_rabbit.memory_config || {};
    const period_ms    = typeof cfg.period_ms    === 'number' ? cfg.period_ms    : 1000;
    const window_ms    = typeof cfg.window_ms    === 'number' ? cfg.window_ms    : 5000;
    const max_archives = typeof cfg.max_archives === 'number' ? cfg.max_archives : MAX_ARCHIVES;
    ...
    if (fs.existsSync(current_path)) {
        const ring_capacity = Math.max(2, Math.ceil(window_ms / period_ms) + 2);
        restored = read_jsonl_tail(current_path, ring_capacity);
        try {
            const archive_path = path.join(storage_dir, archive_name(Date.now()));
        ...
        prune_archives(storage_dir, max_archives);
    }

    const mem = make_memory(white_rabbit, period_ms, window_ms, storage_dir);
    ...
    tick();
    mem.interval = setInterval(tick, period_ms);

    // 4. On graceful shutdown, flush. Use once() so repeated init() calls
    //    (tests, future hot-reload) don't stack duplicate listeners.
...
module.exports = white_rabbit_memory;
