...
// Tell the realsense vision subprocess (over the same stdin channel used for
// pitch/roll/heading, see connect_to_realsense.js start_pitch_stream) to start or
// stop saving its screenshot frames into a specific folder, so the frames for an
// arm session sit right in the same logger/<date>/<count>/ folder as that
// session's rc_edge_capture JSON log, instead of a separate screenshots/ tree.
function send_capture_session(white_rabbit, active, dir) {
	var proc = white_rabbit.realsense && white_rabbit.realsense.process;
	if (!proc || !proc.stdin || proc.stdin.destroyed) return;
	try {
		proc.stdin.write(JSON.stringify({ message: 'capture_session', active: active, dir: dir || null }) + '\n');
	} catch (e) {
		// stdin may be mid-teardown; not worth logging, next arm/disarm will retry
	}
}
...
var pixhawk_message_handler = function (white_rabbit, message) {
	...
	if (message.name == 'HEARTBEAT') {
		...
		if (message.autopilot == 3) {
			...
			if ((message.base_mode & MAV_MODE_FLAG_SAFETY_ARMED) !== 0) {
				if (!white_rabbit.robot_data.is_armed) {
					...
					// Start a fresh RC edge-capture session — a new logger every arm.
					white_rabbit.rc_edge_capture = white_rabbit.rc_edge_capture || { session_count: 0 };
					white_rabbit.rc_edge_capture.session_count++;
					white_rabbit.rc_edge_capture.logger = new white_rabbit.logging(white_rabbit, 'rc_edge_capture_' + white_rabbit.rc_edge_capture.session_count);
					white_rabbit.rc_edge_capture.active = true;
					console.log('RC edge capture: session ' + white_rabbit.rc_edge_capture.session_count + ' started');

					// Frames for this session live IN the same folder as its log (logger/<date>/
					// <count>/), in a subfolder named after the log itself, so the two are
					// grouped together on disk instead of living in two separate trees. Each
					// frame is named frame_<epoch ms>.jpg (unchanged) and each JSON log line now
					// carries its own epoch-ms ts (radio_commands.js) — same unit, so a frame and
					// its surrounding log lines match directly on time.
					white_rabbit.rc_edge_capture.capture_dir = path.join(
						white_rabbit.rc_edge_capture.logger.baseDir,
						'rc_edge_capture_' + white_rabbit.rc_edge_capture.session_count
					);
					send_capture_session(white_rabbit, true, white_rabbit.rc_edge_capture.capture_dir);
				}
			} else if (white_rabbit.robot_data.is_armed) {
				white_rabbit.robot_data.is_armed = false;
				console.log("Vehicle is Disarmed");

				if (white_rabbit.rc_edge_capture && white_rabbit.rc_edge_capture.active) {
					white_rabbit.rc_edge_capture.active = false;
					console.log('RC edge capture: session ' + white_rabbit.rc_edge_capture.session_count + ' stopped');
					send_capture_session(white_rabbit, false, null);
				}
			}
		...
		white_rabbit.robot_data.robot_latitude = parseFloat(white_rabbit.robot_data.GLOBAL_POSITION_INT.lat) / 10000000;
		white_rabbit.robot_data.robot_longitude = parseFloat(white_rabbit.robot_data.GLOBAL_POSITION_INT.lon) / 10000000;
