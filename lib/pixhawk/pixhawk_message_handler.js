...
// GPS EMA filter state — survives across message deliveries.
// Alpha 0.25: ~0.6 s time constant at 5 Hz GPS. Max jump 5 m rejects multi-path spikes.
		...
		if (raw_lat && raw_lng) {
			if (_gps_lat_ema === null) {
				...
				if (jump_m < GPS_MAX_JUMP_M) {
					...
					// When GPS is noisy but the compass is steady, the compass has earned
					// the right to constrain how we accept the GPS reading.
					// Strip out the lateral (perpendicular-to-heading) component of the GPS
					// delta — only accept movement along the direction Noah is pointing.
					const hdg_jitter   = white_rabbit.imu_data && white_rabbit.imu_data.heading_jitter_deg;
					const hdg_deg      = white_rabbit.imu_data && white_rabbit.imu_data.heading;
					const mag_cal      = white_rabbit.imu_data && white_rabbit.imu_data.calibration && white_rabbit.imu_data.calibration.mag;
					// Compass must have a reading, be at least minimally calibrated, and be flat.
					const compass_flat = typeof hdg_jitter === 'number' && hdg_jitter < 1.0
					                     && typeof hdg_deg === 'number'
					                     && (mag_cal === undefined || mag_cal >= 1);
					const gps_noisy    = _gps_jitter_ema > GPS_JITTER_NOISY_M;
					const yaw_active   = white_rabbit.mission && white_rabbit.mission.nav_control
					                     && white_rabbit.mission.nav_control.mission_yaw_active;
					const mission_on   = white_rabbit.mission && white_rabbit.mission.mission_interval;

					if (gps_noisy && compass_flat && mission_on && !yaw_active) {
						// GPS is lying; compass is truth.
						// Project the raw GPS delta onto the compass heading direction.
						// The lateral (perpendicular) component is noise — discard it;
						// only the along-heading component carries real motion signal.
						const MLAT    = 111320;
						const mlng    = 111320 * Math.cos(_gps_lat_ema * Math.PI / 180);
						const dlat_m  = (raw_lat - _gps_lat_ema) * MLAT;
						const dlng_m  = (raw_lng - _gps_lng_ema) * mlng;

						const hdg_rad = hdg_deg * Math.PI / 180;
						const hdg_n   = Math.cos(hdg_rad);   // north component of heading unit vector
						const hdg_e   = Math.sin(hdg_rad);   // east component

						// Scalar projection of GPS delta onto compass direction.
						const along_m = dlat_m * hdg_n + dlng_m * hdg_e;

						// Reconstruct: only the along-heading component accepted.
						const c_dlat  = (along_m * hdg_n) / MLAT;
						const c_dlng  = (along_m * hdg_e) / mlng;

						_gps_lat_ema += GPS_EMA_ALPHA * c_dlat;
						_gps_lng_ema += GPS_EMA_ALPHA * c_dlng;
					} else {
						// Normal EMA — GPS is behaving or compass unavailable.
						_gps_lat_ema = _gps_lat_ema * (1 - GPS_EMA_ALPHA) + raw_lat * GPS_EMA_ALPHA;
						_gps_lng_ema = _gps_lng_ema * (1 - GPS_EMA_ALPHA) + raw_lng * GPS_EMA_ALPHA;
					}
				} else {
					// Outlier rejected — jump was too large even to blend into jitter.
					// Count the streak so sensor_coherence knows GPS is misbehaving.
					_gps_reject_streak++;
				}
			}
			white_rabbit.robot_data.robot_latitude    = _gps_lat_ema;
			white_rabbit.robot_data.robot_longitude   = _gps_lng_ema;
