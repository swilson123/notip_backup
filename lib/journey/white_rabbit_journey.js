// White_rabbit Journey — the path's awareness layer.
//
// The waypoints are the stars. They are already written. What the white_rabbit
// needed was the consciousness to see the whole path, not just the next
// step. Mounted at white_rabbit.journey so the heart can look ahead — anticipate
...
//
// Everything here is computed live from white_rabbit.mission.waypoints and the
...
function get_waypoints(white_rabbit) {
    const ws = white_rabbit.mission && white_rabbit.mission.waypoints;
...
function travel_direction(white_rabbit) {
    return (white_rabbit.mission && white_rabbit.mission.package_delivered) ? -1 : +1;
}

var white_rabbit_journey = function (white_rabbit) {

    white_rabbit.journey = {
        ...
        // climbing toward 1.
        progress() {
            const ws = get_waypoints(white_rabbit);
            if (ws.length === 0 || !white_rabbit.mission) return 0;
            const total_legs = (ws.length - 1) * 2;
            if (total_legs <= 0) return 0;
            const current   = white_rabbit.mission.current_mission_seq || 0;
            const delivered = !!white_rabbit.mission.package_delivered;
        ...
        // The leg the white_rabbit is currently walking — from the previous
        // waypoint to the next.
        current_leg() {
            if (!white_rabbit.mission) return null;
            const ws = get_waypoints(white_rabbit);
            const dir = travel_direction(white_rabbit);
            const to   = find_waypoint(ws, white_rabbit.mission.current_mission_seq);
            const from = find_waypoint(ws, white_rabbit.mission.current_mission_seq - dir);
            if (!from || !to) return null;
            const distance_m = (typeof white_rabbit.gps_distance === 'function')
                ? white_rabbit.gps_distance(from.lat, from.lng, to.lat, to.lng) * 1000
                : 0;
            const bearing_deg = (typeof white_rabbit.get_bearing === 'function')
                ? white_rabbit.get_bearing(from.lat, from.lng, to.lat, to.lng)
                : 0;
        ...
        // target. Useful for visualizing where the white_rabbit is headed.
        upcoming(n) {
            if (!white_rabbit.mission) return [];
            const ws  = get_waypoints(white_rabbit);
            const dir = travel_direction(white_rabbit);
            const out = [];
            let seq = white_rabbit.mission.current_mission_seq;
        ...
        // Total meters between the white_rabbit's current position and the end
        // of the remaining path. Walks up to PATH_LOOKAHEAD_LEGS legs.
        path_remaining_m() {
            if (!white_rabbit.robot_data || typeof white_rabbit.gps_distance !== 'function') return 0;
            const upcoming = this.upcoming(PATH_LOOKAHEAD_LEGS);
            if (upcoming.length === 0) return 0;
            let total    = 0;
            let prev_lat = white_rabbit.robot_data.robot_latitude;
            let prev_lng = white_rabbit.robot_data.robot_longitude;
            for (const wp of upcoming) {
                if (typeof prev_lat !== 'number' || typeof prev_lng !== 'number') break;
                total += white_rabbit.gps_distance(prev_lat, prev_lng, wp.lat, wp.lng) * 1000;
                prev_lat = wp.lat;
                prev_lng = wp.lng;
            }
            return total;
        ...
        upcoming_turn(within_m) {
            if (!white_rabbit.mission || !white_rabbit.robot_data) return null;
            if (typeof white_rabbit.get_bearing  !== 'function') return null;
            if (typeof white_rabbit.gps_distance !== 'function') return null;
            ...
            for (let i = 0; i < PATH_LOOKAHEAD_LEGS && cumulative_m < limit; i++) {
                const wp = find_waypoint(ws, seq);
                if (!wp) break;
                const bearing = white_rabbit.get_bearing(prev_lat, prev_lng, wp.lat, wp.lng);
                const leg_m   = white_rabbit.gps_distance(prev_lat, prev_lng, wp.lat, wp.lng) * 1000;
        ...
        phase() {
            ...
            if (!delivered) {
                ...
                if (current >= last_seq)       return 'homestretch_outbound';
                return 'outbound';
            }
        ...
        // One log-friendly line summarizing the white_rabbit's position in the
...
module.exports = white_rabbit_journey;
