...
async function sendData(bus, addr, char) {
    await sendByte(bus, addr, char, RS_DATA);
}
...
async function initLCD(bus, addr) {
    ...
    await sendCommand(bus, addr, 0x28); // function set: 4-bit, 2 lines, 5x8
...
function makeLCDInstance(bus, addr) {
    ...
    return {
        ...
        writeString: async (str) => {
            for (const ch of str.slice(0, 16)) {
...
var connect_to_lcd = async function (rover) {
    const bus = rover.i2c.openSync(1);

    for (const key of ['screen1', 'screen2', 'screen3']) {
        const screen = rover.lcd_screens[key];
        try {
    ...
    if (!rover.lcd_screens.write_to_lcd_interval) {
        rover.lcd_screens.write_to_lcd_interval = setInterval(() => {
            rover.write_to_lcd(rover);
        }, 1000);
    }
};
