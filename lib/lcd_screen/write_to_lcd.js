...
// Wavy mouth: a single horizontal sine wave rippling across the panel. Shown when
// GPS quality is poor (high HDOP or low satellite count) — a wobbling, "uncertain"
// mouth distinct from the steady smile/frown. The phase advances each tick so the
// wave travels sideways like a nervous quiver.
const WAVY_Y_CENTER = 15.5;   // vertical center of the panel, in pixels (0..31)
const WAVY_AMP_PX    = 9;     // peak deviation above/below center
const WAVY_FREQ      = 0.85;  // spatial frequency across columns
const WAVY_SPEED     = 0.9;   // phase advance per tick

function compute_wavy_grid(tick) {
    const grid = blank_grid();
    for (let c = 0; c < COLS; c++) {
        const y = WAVY_Y_CENTER + WAVY_AMP_PX * Math.sin(c * WAVY_FREQ + tick * WAVY_SPEED);
        place_stroke(grid, c, y);
    }
    return grid;
}
...
async function draw_mouth(screen, shape, tick, speaking) {
    ...
    let grid;
    if (speaking)            grid = compute_talk_grid(tick);
    else if (shape === 'wavy') grid = compute_wavy_grid(tick);
    else                     grid = compute_mouth_grid(shape, tick);
...
function all_systems_good(white_rabbit) {
    ...
    const imu_ok = !(white_rabbit.imu && white_rabbit.imu.enabled !== false) || white_rabbit.imu_data.connected;
...
// GPS quality gate for the wavy mouth: true when the fix is degraded — either HDOP
// is above gps_warn_hdop or the satellite count is below gps_warn_min_sats. Unknown
// readings (no GPS sample yet) do not trigger it, so the face stays calm at boot
// until real telemetry arrives.
function gps_quality_poor(white_rabbit) {
    const nt = white_rabbit.nav_tuning || {};
    const warn_hdop = typeof nt.gps_warn_hdop === 'number' ? nt.gps_warn_hdop : 2.0;
    const warn_sats = typeof nt.gps_warn_min_sats === 'number' ? nt.gps_warn_min_sats : 6;
    const hdop = gps_hdop(white_rabbit);
    const sats = sats_visible(white_rabbit);
    if (hdop != null && hdop > warn_hdop) return true;
    if (sats != null && sats < warn_sats) return true;
    return false;
}

function heading_deg(white_rabbit) {
    // Show the same offset-applied heading nav uses — get_heading adds
    // pixhawk_offset_deg to the Pixhawk heading (and the IMU heading already has
    // compass_offset_deg baked in). Return null only when there's no heading
    // source at all, so the display reads '---' rather than a fake 0.
    ...
    if (typeof white_rabbit.get_heading === 'function') return white_rabbit.get_heading(white_rabbit);
    return have_vfr ? white_rabbit.robot_data.VFR_HUD.heading : white_rabbit.imu_data.heading;
}
...
async function draw_status(screen, white_rabbit) {
    ...
    // IMU segment (hidden when the IMU is disabled) and mission progress for the row
    // directly above the heading: WP<seq>/<total> = current sequence of total waypoints.
    const imu_seg  = (white_rabbit.imu && white_rabbit.imu.enabled !== false)
        ? 'IMU:' + ok_str(white_rabbit.imu_data.connected) + ' ' : '';
    const seq      = (white_rabbit.mission && typeof white_rabbit.mission.current_mission_seq === 'number') ? white_rabbit.mission.current_mission_seq : 0;
    const wp_total = (white_rabbit.mission && typeof white_rabbit.mission.mission_count === 'number') ? white_rabbit.mission.mission_count : 0;

    // IRL moved up to the WAVE/ARD row to free the row above the heading for WP/Seq.
    const lines = [
        `LIDAR:${ok_str(white_rabbit.rplidar.connected)}  PIX:${ok_str(white_rabbit.pixhawk_port.connected)}`,
        `WAVE:${ok_str(wave_ok)} ARD:${ok_str(white_rabbit.arduino.connected)} IRL:${irl_conn}`,
        `RS:${ok_str(white_rabbit.realsense.connected)} ${imu_seg}WP${seq}/${wp_total}`,
...
var write_to_lcd = async function (white_rabbit) {
    ...
    if (white_rabbit.lcd_screens.screen1.connected) {
        try {
            // Hardware failure (frown) is the most urgent and wins. Otherwise, if all
            // subsystems are up but GPS quality is degraded, ripple a wavy mouth; else smile.
            let shape;
            if (!all_systems_good(white_rabbit))      shape = 'frown';
            else if (gps_quality_poor(white_rabbit))  shape = 'wavy';
            else                                      shape = 'smile';
