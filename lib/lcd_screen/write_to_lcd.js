...
async function draw_realsense(screen, white_rabbit) {
    ...
    // 00 = edge not seen; -- = seen but confidence < 50%; NN = confidence value
    ...
    // low-confidence flags: edge seen but below 50% — X/Y show '--' instead of a value
    const el_low = el_seen && (typeof det.edge_left_conf !== 'number' || det.edge_left_conf < 0.5);
    const er_low = er_seen && (typeof det.edge_right_conf !== 'number' || det.edge_right_conf < 0.5);

    // Prefer explicit per-edge X/Y telemetry; keep older fields as fallback.
    // '----' = edge not seen; '--' = seen but low confidence; '+N.N' = value
    function fmt_m_1(v, seen, low_conf) {
    ...
    const left_x = fmt_m_1((typeof det.edge_left_x_m === 'number') ? det.edge_left_x_m : det.edge_left_m, el_seen, el_low);
    const left_y = fmt_m_1((typeof det.edge_left_y_m === 'number') ? det.edge_left_y_m : det.edge_forward_m, el_seen, el_low);
    const right_x = fmt_m_1((typeof det.edge_right_x_m === 'number') ? det.edge_right_x_m : det.edge_right_m, er_seen, er_low);
    const right_y = fmt_m_1((typeof det.edge_right_y_m === 'number') ? det.edge_right_y_m : det.edge_forward_m, er_seen, er_low);
