...
async function draw_realsense(screen, white_rabbit) {
    ...
    // Edge detected flags (1/0) gated on freshness
    const el_vis = !!(fresh && det.left_boundary_visible);
    const er_vis = !!(fresh && det.right_boundary_visible);

    // Per-edge "confidence" = overall detection confidence if that edge is visible
    const el_conf = el_vis && typeof det.confidence === 'number' ? det.confidence.toFixed(2) : '----';
    const er_conf = er_vis && typeof det.confidence === 'number' ? det.confidence.toFixed(2) : '----';

    // Per-edge clearance distance: sign+1-decimal fixed width, e.g. "+1.2m" or "----m"
    function fmt_clr(m) {
        if (typeof m !== 'number') return '----m';
        const sign = m >= 0 ? '+' : '-';
        return sign + Math.abs(m).toFixed(1) + 'm';
    }
    const ld_str = fmt_clr(det.left_edge_clearance_m);   // 6 chars e.g. "+1.2m"
    const rd_str = fmt_clr(det.right_edge_clearance_m);  // 6 chars
    ...
    // 20-char rows:
    // Row 0: "EL:1 c=0.85 L:+1.2m"  (edge-left flag, conf, left clearance)
    // Row 1: "ER:0 c=---- R:----m " (edge-right flag, conf, right clearance)
    // Row 2: "AV:CLR L:G V:G IRL:1" (avoidance, lidar, vision OBJ, irlock)
    // Row 3: blank
    const lines = [
        `EL:${el_vis ? '1' : '0'} c=${el_conf} L:${ld_str}`,
        `ER:${er_vis ? '1' : '0'} c=${er_conf} R:${rd_str}`,
