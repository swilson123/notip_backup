...
async function draw_realsense(screen, white_rabbit) {
    ...
    // Confidence is the single seen-indicator: a detected edge always reports
    // >= ~55, so 00 unambiguously means "no edge" (no separate Y/N flag needed).
    ...
    // When the edge isn't seen, X/Y read '----' so the row never shows stale values.
