...
function all_systems_good(white_rabbit) {
    ...
    return white_rabbit.pixhawk_port.connected
        && wave_ok
...
async function draw_realsense(screen, white_rabbit, opts) {
    // opts.rows_only: if set, only write the listed row indices (for fast partial refresh).
    const lcd = screen.instance;
    ...
    // carrot_angle_deg straight from carrot_vision.py (via sidewalk_vision.py's x_angle_deg) --
    // the raw vision-computed angle BEFORE carrot.js's clamp/smoothing. Compare against the
    // A: field on row 3, which is the final commanded angle actually sent to the servos.
    const raw_angle = (fresh && typeof det.x_angle_deg === 'number') ? det.x_angle_deg : null;
    const raw_angle_str = raw_angle != null ? (raw_angle >= 0 ? '+' : '') + raw_angle.toFixed(1) : '---';

    // sidewalk_vision.py's edge-detection confidence (0-1 -> 0-100%).
    const conf_pct = (fresh && typeof det.confidence === 'number') ? Math.round(det.confidence * 100) : null;
    const conf_str = conf_pct != null ? String(conf_pct).padStart(3, ' ') + '%' : '---';
    ...
    const vf_flag = (white_rabbit.mission && white_rabbit.mission.sidewalk_follow_active) ? 'Y' : 'N';

    // Row 3: commanded steering angle + GPS fix state + horizontal precision (HDOP).
    // Shows the final tuned angle sent to the servos, or '---' when not in sidewalk mode.
    const ang_raw = white_rabbit.motor && white_rabbit.motor.steering_angle_deg;
    const ang_str = (typeof ang_raw === 'number' && isFinite(ang_raw))
        ? (ang_raw >= 0 ? '+' : '') + ang_raw.toFixed(1)
        : '---';
    ...
    // 20-char rows:
    // Row 0: "ANGLE: +12.3"        (carrot_angle_deg -- raw vision angle from carrot_vision.py)
    // Row 1: "CONFIDENCE:  87%"    (sidewalk_vision.py's edge-detection confidence)
    // Row 2: "AV:CLR L:G V:G VF:Y" (avoidance, lidar, vision OBJ, sidewalk gate Y/N)
    // Row 3: "A:+12.5 RTKfix H0.6" (final commanded steering angle, GPS fix state, HDOP)
    const lines = [
        `ANGLE: ${raw_angle_str}`,
        `CONFIDENCE: ${conf_str}`,
        `AV:${avoid_str} L:${front_status} V:${vision_status} VF:${vf_flag}`,
        `A:${ang_str} ${gps_fix} H${hdop_str}`
    ];

    const rows_only = opts && opts.rows_only;
    for (let r = 0; r < ROWS; r++) {
        if (rows_only && !rows_only.includes(r)) continue;
        await lcd.setCursor(0, r);
        await lcd.writeString(pad(lines[r], COLS));
    }
}
...
var write_to_lcd = async function (white_rabbit) {
    ...
    // The i2c loop ticks at ~100ms (see connect_to_lcd.js). Every screen refreshes every
    // CADENCE ticks (~3Hz), but their phases are STAGGERED (offset 0/1/2) so only one screen
    // writes per tick — the serialized i2c bus stays smooth instead of bunching all three
    // heavy writes onto the same tick. All draws share the one serialized loop, so the i2c
    // bus is never written concurrently.
    const CADENCE = 3;  // every 3 ticks ≈ 3 Hz, for each screen

    // LCD 3 — RealSense vision. Edge data + steering angle (rows 0,1,3) refresh every
    // tick (~10Hz). The slower-changing status row (row 2) refreshes at the normal cadence.
    if (white_rabbit.lcd_screens.screen3.connected) {
        try {
            const full_refresh = (tick % CADENCE) === 0;
            await draw_realsense(
                white_rabbit.lcd_screens.screen3,
                white_rabbit,
                full_refresh ? null : { rows_only: [0, 1, 3] }
            );
        } catch (err) {
    ...
    // LCD 1 — animated mouth (~3Hz, phase 1). Pass the cadence-divided tick so the animation
    // advances one frame per refresh.
    if ((tick % CADENCE) === 1 && white_rabbit.lcd_screens.screen1.connected) {
        try {
            ...
            await draw_mouth(white_rabbit.lcd_screens.screen1, shape, Math.floor(tick / CADENCE), speaking);
        } catch (err) {
    ...
    // LCD 2 — device/battery/GPS/heading status (~3Hz, phase 2).
    if ((tick % CADENCE) === 2 && white_rabbit.lcd_screens.screen2.connected) {
        try {
