...
async function draw_realsense(screen, white_rabbit) {
    ...
    // vision_fresh flag and authority from the mission nav_control state
    const _nc = white_rabbit.mission && white_rabbit.mission.nav_control;
    const vf_flag = (_nc && _nc.sidewalk_vision_fresh) ? 'Y' : 'N';
    const va_pct  = (_nc && typeof _nc.sidewalk_authority === 'number')
        ? Math.round(_nc.sidewalk_authority * 100) : 0;
    const va_str  = String(Math.min(99, va_pct)).padStart(2, '0');

    // 20-char rows:
    // Row 0: "EL C85 X+0.5 Y+0.7"  (edge seen — confidence + position)
    // Row 1: "ER C00 X---- Y----"  (edge not seen — C00, X/Y dashes)
    // Row 2: "AV:CLR L:G V:G VF:Y" (avoidance, lidar, vision OBJ, vision-fresh Y/N)
    // Row 3: "GPS:RTKfix H:0.6 A99" (fix, HDOP, vision authority %)
    const lines = [
        `EL C${el_conf} X${left_x} Y${left_y}`,
        `ER C${er_conf} X${right_x} Y${right_y}`,
        `AV:${avoid_str} L:${front_status} V:${vision_status} VF:${vf_flag}`,
        `GPS:${gps_fix_str} H:${hdop_str} A${va_str}`
    ];
...
var write_to_lcd = async function (white_rabbit) {
    ...
    // LCD 3 — RealSense vision (~3Hz, phase 0). Drawn first so the operator-facing readout
    // has priority on its tick.
    if ((tick % CADENCE) === 0 && white_rabbit.lcd_screens.screen3.connected) {
        try {
