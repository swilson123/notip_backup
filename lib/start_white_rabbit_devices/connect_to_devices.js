var connect_to_devices = function (white_rabbit) {
    ...
    if (white_rabbit.imu && white_rabbit.imu.enable_imu && white_rabbit.imu_data.connected === false) {
        ...
        } else if (white_rabbit.imu.compass_type === 'witmotion_hwt905') {
            white_rabbit.connect_to_witmotion_hwt905(white_rabbit); // WitMotion HWT905-TTL over UART
        } else {
            white_rabbit.connect_to_imu(white_rabbit);        // BNO055 (rollback)
        }
    }

    //WitMotion MJRTK-UM982 dual-antenna RTK GNSS. Stays a no-op until witmotion_mjrtk_comName
    //is filled in, so an unconfigured rover boots exactly as before.
    if (white_rabbit.witmotion.port_path && white_rabbit.witmotion.connected === false) {
        white_rabbit.connect_to_witmotion_mjrtk(white_rabbit);
    }
