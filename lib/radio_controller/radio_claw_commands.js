...
var radio_claw_commands = function (rover, message) {
	...
	if (message.chan5_raw > 1900 && rover.claw.rc_claw != message.chan5_raw) {
		//Manual Claw Control On...............................................................
		rover.claw.rc_claw = message.chan5_raw;
		console.log("RC Claw Manual Control Activated");
	}
	else if (message.chan5_raw < 1100 && rover.claw.rc_claw != message.chan5_raw) {
  ...
  //rover.create_arduino_message(rover, 'deliver_package', 0);
	...
	if (rover.claw.rc_claw > 1900) {
		...
		if (rover.claw.rc_actuator != message.chan8_raw) {
			...
			if (rover.claw.rc_telescope != message.chan2_raw) {
			rover.claw.rc_telescope = message.chan2_raw;
			//send arduino command to command actuator
			console.log("Send arduino telescope command");
			rover.create_arduino_message(rover, 'telescope', convertActuatorValue(rover.claw.rc_telescope));
		}
