...
var radio_claw_commands = function (white_rabbit, message) {
	...
	if (message.chan5_raw > 1900 && white_rabbit.claw.rc_claw != message.chan5_raw) {
		...
		white_rabbit.nav_tuning.sidewalk_gate_always_open = true;
	}
		...
		white_rabbit.nav_tuning.sidewalk_gate_always_open = false;

		if (!white_rabbit.robot_data.mission_mode && white_rabbit.robot_data.is_armed) {
			//if in rc mode and run deliver package seq
	...
	if (white_rabbit.claw.rc_claw > 1900 && !white_rabbit.mission.auto_delivery) {
		...
		if (white_rabbit.claw.rc_telescope != message.chan12_raw) {
			white_rabbit.claw.rc_telescope = message.chan12_raw;
			//send arduino command to command actuator
		...
		//Belt Control............
		if (white_rabbit.claw.rc_belt != message.chan2_raw) {
			white_rabbit.claw.rc_belt = message.chan2_raw;
