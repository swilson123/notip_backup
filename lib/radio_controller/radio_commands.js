var radio_commands = function (rover, message) {
	...
	rover.radio_dock_commands(rover, message);
	...
	if (message.chan11_raw < 1500 && !rover.robot_data.mission_mode) {
		...
		if (rover.mission.mission_interval) {
	...
	if (!rover.robot_data.mission_mode && Date.now() < (rover.voice_override_until || 0)) {
		var stick_deflected =
		...
		if (stick_deflected) {
			...
			if (rover.mission) rover.mission.pause_mission = false;
	...
	if (!rover.robot_data.mission_mode) {
		...
		if (!rover.rc_contoller.pause_cmd) {
			...
			if (message.chan4_raw > 1450 && message.chan4_raw < 1550) {
				...
				rover.motor.steering_angle_deg = rover.pwm_to_angle(message.chan1_raw);

				//console.log("Steering Angle Deg: ", rover.motor.steering_angle_deg);


				var steering_and_rpm = rover.calc_steering_and_rpm(rover, rover.motor.steering_angle_deg, motor_speed_cmd);
				...
				rover.yaw_rover(rover, yaw_degrees, yaw_motor_speed_cmd);
