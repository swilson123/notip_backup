var radio_commands = function (white_rabbit, message) {
	...
	// RC edge-capture logging — active only during a hand-driven data-collection
	// session (started/stopped by the chan11 arm/disarm switch below). Sampled off
	// every valid RC frame rather than the mission tick, so it keeps recording
	// steering/throttle commands even while the sticks are driving directly.
	if (white_rabbit.rc_edge_capture && white_rabbit.rc_edge_capture.active && white_rabbit.rc_edge_capture.logger) {
		...
		white_rabbit.rc_edge_capture.logger.log(white_rabbit, {
			...
			steer_deg:    white_rabbit.pwm_to_angle(message.chan1_raw),
