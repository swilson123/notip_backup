var radio_commands = function (white_rabbit, message) {
	...
	if (message.chan11_raw >= 1500) {
		// Rising edge of the mission switch — pull a FRESH mission from the Pixhawk.
		if (!white_rabbit.rc_contoller.mission_switch_armed) {
			white_rabbit.rc_contoller.mission_switch_armed = true;

			// Clear the old waypoints and reset the count so the MISSION_COUNT handler
			// (which only re-downloads when the count changes) always re-downloads.
			white_rabbit.mission.waypoints = [];
			white_rabbit.mission.mission_count = 0;
			white_rabbit.mission.current_mission_seq = 0;
			white_rabbit.mission._mission_received_announced = false;

			var _mreq = white_rabbit.mavlink_messages.MISSION_REQUEST_LIST(white_rabbit);
			white_rabbit.send_pixhawk_command(white_rabbit, _mreq[0], _mreq[1], null);
			console.log("Mission switch ON — requesting fresh mission download from Pixhawk");
		}
		white_rabbit.rc_contoller.mission_start_pending = false;
	}
	...
	if (!white_rabbit.robot_data.mission_mode) {
		...
		if (!white_rabbit.rc_contoller.pause_cmd) {
			...
			// Publish the live RC drive command so compass auto-calibration knows
			// the rover is actively driving and whether it's forward or reverse.
			white_rabbit.motor.motor_speed_cmd    = motor_speed_cmd;
			white_rabbit.motor.motor_speed_cmd_ts = Date.now();


			// Throttle and yaw share one stick gimbal, so a small yaw deflection
			// while pushing throttle would instantly kick the rover into the
			// spin-in-place yaw branch. Require chan4 to clear a wider deadband
			// (rc_yaw_deadband_pwm, default ±150 from 1500) before yawing.
			var yaw_deadband = (white_rabbit.nav_tuning && typeof white_rabbit.nav_tuning.rc_yaw_deadband_pwm === 'number')
				? white_rabbit.nav_tuning.rc_yaw_deadband_pwm
				: 150;

			if (message.chan4_raw > (1500 - yaw_deadband) && message.chan4_raw < (1500 + yaw_deadband)) {
