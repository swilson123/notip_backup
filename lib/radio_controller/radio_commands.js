var radio_commands = function (white_rabbit, message) {
	...
	if (white_rabbit.rc_edge_capture && white_rabbit.rc_edge_capture.active && white_rabbit.rc_edge_capture.logger) {
		...
		white_rabbit.rc_edge_capture.logger.log(white_rabbit, {
			// Epoch ms, same unit/clock as the frame_<epoch ms>.jpg screenshots
			// sidewalk_vision.py saves to display_capture_dir -- the logger line itself
			// is only timestamped to the second, so this is what actually lets a frame
			// be matched to the log lines around it.
			ts: Date.now(),
			...
			x_angle_deg: _capture_pd.x_angle_deg,
	...
	if (
		...
		!white_rabbit.robot_data.mission_mode && white_rabbit.rc_contoller.mission_switch != message.chan11_raw
	) {
		...
		console.log("Mission mode enabled by RC");

	}
	else if (message.chan11_raw >= 1500 && white_rabbit.rc_contoller.mission_switch != message.chan11_raw) {
	...
	white_rabbit.rc_contoller.mission_switch = message.chan11_raw;
	...
	if (!white_rabbit.robot_data.mission_mode) {
		...
		if (!white_rabbit.rc_contoller.pause_cmd) {
			...
			if (message.chan4_raw > (1500 - white_rabbit.nav_tuning.rc_yaw_deadband_pwm) && message.chan4_raw < (1500 + white_rabbit.nav_tuning.rc_yaw_deadband_pwm)) {
