...
var radio_dock_commands = function (white_rabbit, message) {
    ...
    if (message.chan7_raw > 1900 && white_rabbit.dock.rc_dock != message.chan7_raw) {
        ...
        // If the arm isn't already stowed, stow it before docking so it can't catch
        // on the ramp/dock. The Arduino heartbeat reports `stowed`; arduino_message_handler
        // clears awaiting_stow_ack once it confirms stowed=true.
        var is_truthy = function (v) { return v === true || v === 1 || v === '1' || v === 'true'; };
        var arm_stowed = white_rabbit.arduino && white_rabbit.arduino.received_data
            && is_truthy(white_rabbit.arduino.received_data.stowed);
        if (!arm_stowed) {
            console.log("Dock: arm not stowed — sending stow_arm to Arduino");
            ...
            if (white_rabbit.voice) white_rabbit.voice.say('Stowing arm.');
        } else {
            white_rabbit.dock.awaiting_stow_ack = false;
        ...
        // Off during mission: keep mission running, but require explicit dock
        // command before final docking.
        if (white_rabbit.robot_data.mission_mode) {
