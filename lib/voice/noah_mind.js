...
function compose_dream(white_rabbit) {
    // Return the most recent stored dream vision if one exists.
    if (white_rabbit.dreams && typeof white_rabbit.dreams.latest === 'function') {
        const d = white_rabbit.dreams.latest();
        if (d && d.vision) return d.vision;
    }
    // Fallback: heart-mood one-liner when no dream has been stored yet.
