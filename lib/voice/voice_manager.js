...
function handle_command(white_rabbit, tts, cmd) {
    ...
    if (intent === 'set_compass') {
        const deg = payload && typeof payload.degrees === 'number' ? payload.degrees : null;
        if (deg === null) {
            tts.speak('I did not catch the heading. Say, set compass to a number.');
            return;
        }
        if (!white_rabbit.imu) {
            tts.speak('The compass is not enabled.');
            return;
        }
        const applied = white_rabbit.compass_calibration
            && typeof white_rabbit.compass_calibration.set_heading === 'function'
            ? white_rabbit.compass_calibration.set_heading(white_rabbit, deg)
            : null;
        if (applied === null) {
            tts.speak('I cannot set the compass without a heading reading.');
            return;
        }
        tts.speak('Compass set. My heading is now ' + Math.round(((deg % 360) + 360) % 360) + ' degrees.');
        return;
    }
...
function drive_distance_closed_loop(white_rabbit, target_m, dir_sign, after_finish_fn, trigger) {
    ...
    // Two-phase speed profile: cruise at full RPM, then ramp down to a creep
    // speed when within the slow-down window. This cuts stopping overshoot from
    // ~20mm (at 30 RPM, 50ms tick) down to ~5mm (at 8 RPM, 50ms tick).
    // nudge_creep_rpm  — slow speed for the final approach (default: rpm/4, min 5)
    // nudge_slowdown_m — distance from target to start creeping (default: 0.05m = 2")
    const creep_rpm    = cfg.nudge_creep_rpm   || Math.max(5, Math.round(rpm / 4));
    const slowdown_m   = cfg.nudge_slowdown_m  !== undefined ? cfg.nudge_slowdown_m : 0.05;
    const slowdown_pulses = slowdown_m * pulses_per_m;
    ...
    white_rabbit.voice_nudge_timeout = setInterval(() => {
        ...
        const remaining_pulses = target_pulses - avg_abs_delta;
        ...
        // Switch to creep speed when within slowdown window.
        const current_rpm = (slowdown_pulses > 0 && remaining_pulses <= slowdown_pulses)
            ? creep_rpm : rpm;
        if (current_rpm === creep_rpm && !in_creep) {
            in_creep = true;
        }

        // Re-assert motor command each tick — defensive against losing it
        // to any other writer that might have slipped in.
        drive_straight(white_rabbit, current_rpm, dir_sign, trigger);
...
function nudge_forward(white_rabbit, meters, after_finish_fn, trigger) {
    const sign = meters >= 0 ? 1 : -1;
    drive_distance_closed_loop(white_rabbit, Math.abs(meters), sign, after_finish_fn || null, trigger || 'voice nudge');
}
...
var white_rabbit_voice = function (white_rabbit) {
    ...
    if (cfg.asr_enabled !== false) {
        ...
        // Two-stage wake: "Hey Noah" → Noah says "Yes." and opens a command window.
        // If a command follows in the same utterance, it is also accepted immediately.
        const WAKE_WINDOW_MS = (cfg.wake_window_ms != null ? cfg.wake_window_ms : 6000);
        let wake_active = false;
        let wake_timer  = null;

        function activate_wake() {
            if (wake_timer) clearTimeout(wake_timer);
            wake_active = true;
            speak_announcement('Yes.', true);
        ...
        function deactivate_wake() {
            if (wake_timer) clearTimeout(wake_timer);
            wake_active = false;
            wake_timer  = null;
        }

        function dispatch_command(cmd) {
        ...
        asr_handle.emitter.on('final', ({ text }) => {
            ...
            if (matched_wake) {
                // Wake phrase heard — strip it and check for an inline command.
                ...
                if (cmd) {
                    // "Hey Noah, stop" — wake + command in one breath.
                    deactivate_wake();
                    speak_announcement('Yes.', true);
                    dispatch_command(cmd);
                } else {
                    // "Hey Noah" alone — open the command window.
                    activate_wake();
                }
                return;
            }
            ...
            if (wake_active) {
                // We are in the wake window — treat the whole utterance as a command.
                console.log('voice: wake-active utterance →', JSON.stringify(norm));
                const cmd = parse_command(norm);
                if (cmd) {
                    deactivate_wake();
                    dispatch_command(cmd);
                }
                // If unrecognised, keep the window open until it times out.
            }
        });
    ...
    white_rabbit.voice = {
        ...
        // Same closed-loop odometry drive the "move forward" voice command uses.
        // Exposed so run_mission can trigger it directly (e.g. the dock-arrival
        // overrun) — after_finish_fn runs once the target distance is reached,
        // it stalls, or it times out.
        nudge_forward(meters, after_finish_fn, trigger) { nudge_forward(white_rabbit, meters, after_finish_fn, trigger); },
        tts,
        stop() {
            tts.stop();
