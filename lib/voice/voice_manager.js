...
// Closed-loop straight drive using ZLAC8015D RPM feedback as wheel odometry.
//
// Each tick: read the four actual_rpm_by_id values (set by
// waveshare_message_handler from the Modbus poll), average their absolute
// magnitudes, and integrate `avg_|rpm| / 60 × wheel_circ_m × dt_s` into
// `distance_m`. Stop when distance_m ≥ target_m, on hard cap, or on stall
// (commanded but motors not turning).
//
// Slip caveat: this measures wheel rotation, not chassis travel — if the
// wheels spin without traction, distance is overestimated. Indoors on hard
// floors that's negligible. For sub-cm accuracy you'd need ZLAC8015D
// position registers (REG_*_POS) instead of velocity.
function drive_distance_closed_loop(rover, target_m, dir_sign, after_finish_fn, trigger) {
    ...
    let last_tick_ts     = Date.now();

    rover.voice_nudge_timeout = setInterval(() => {
        const now    = Date.now();
        const dt_s   = (now - last_tick_ts) / 1000;
        last_tick_ts = now;

        const fb = rover.zling.actual_rpm_by_id || {};
        const wheels = [fb[1], fb[2], fb[3], fb[4]].filter(v => typeof v === 'number');
        const avg_abs_rpm = wheels.length
            ? wheels.reduce((s, v) => s + Math.abs(v), 0) / wheels.length
            : 0;

        distance_m += (avg_abs_rpm / 60) * wheel_circ_m * dt_s;
        if (avg_abs_rpm > 1) last_motion_ts = now;

        // Re-assert motor command each tick — defensive against losing it to
        // any other writer that might have slipped in.
        drive_straight(rover, rpm, dir_sign, trigger);

        const reached  = distance_m >= target_m;
