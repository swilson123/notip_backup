...
function nudge_lateral(rover, dir, meters) {
    // Forward arc with steering bias — not true sideways motion.
    // Real crab movement would need 4-wheel crab mode.
    const rpm  = (rover.voice_config && rover.voice_config.nudge_speed_rpm) || DEFAULT_NUDGE_RPM;
    const mps  = (rover.voice_config && rover.voice_config.nudge_mps)       || DEFAULT_NUDGE_MPS;
...
// Tank-style spin. With `degrees`, runs a closed loop on ATTITUDE.yaw and
// stops when the unwrapped delta reaches the target. Without it, falls back
// to the legacy 1.5 s timed spin.
function nudge_spin(rover, dir, degrees) {
    ...
    rover.move_rover(rover, 1,  rpm * sign, 'voice spin angle');
    rover.move_rover(rover, 2,  rpm * sign, 'voice spin angle');
    rover.move_rover(rover, 3, -rpm * sign, 'voice spin angle');
    rover.move_rover(rover, 4, -rpm * sign, 'voice spin angle');
