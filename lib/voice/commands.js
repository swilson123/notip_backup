...
const COMMANDS = [
    ...
    { patterns: [/move\s+left|go\s+left/],                                       intent: 'move_left',    parse: parse_distance },
    { patterns: [/move\s+right|go\s+right/],                                     intent: 'move_right',   parse: parse_distance },
    { patterns: [/go\s+forward|move\s+forward|forward|advance/],                 intent: 'move_forward', parse: parse_distance },
    { patterns: [/back\s*up|reverse|go\s+back|move\s+back/],                    intent: 'move_back',    parse: parse_distance },
    { patterns: [/spin\s+left|rotate\s+left/],                                   intent: 'spin_left' },
    { patterns: [/spin\s+right|rotate\s+right/],                                 intent: 'spin_right' },

    // Mission control
    { patterns: ['stop', 'halt', 'freeze', 'emergency stop'],                    intent: 'stop',          urgent: true },
    { patterns: ['abort', 'abort mission', 'cancel'],                            intent: 'abort',         urgent: true },
    { patterns: ['resume', 'go', 'continue', 'move out', 'carry on'],           intent: 'resume' },
    { patterns: ['return home', 'go home', 'come home', 'rtl', 'return'],       intent: 'return_home' },
    { patterns: ['deliver', 'drop package', 'deploy package', 'drop it'],       intent: 'deliver' },

    // Diagnostics
    { patterns: [/status|what.*doing|how.*doing/],                               intent: 'status' },
    { patterns: [/where.*you|your\s+location|gps|coordinates/],                 intent: 'location' },
