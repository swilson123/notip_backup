...
var arduino_message_handler = function (white_rabbit, data) {
	...
	try {
		...
		if (white_rabbit.arduino.received_data.auto_delivery == 0 && white_rabbit.mission.auto_delivery) {
			...
			if (white_rabbit.dock) white_rabbit.dock.awaiting_stow_ack = true;
		...
		if (white_rabbit.dock && white_rabbit.dock.awaiting_stow_ack && is_truthy(white_rabbit.arduino.received_data.stowed)) {
			...
			if (!white_rabbit.dock.stow_confirmed) {
				...
				if (white_rabbit.voice) white_rabbit.voice.say('Arm stowed.');
				// Arm is stowed. run_mission's dock_return_phase sequence handles navigation
				// back to the dock and starting dock_white_rabbit when Noah arrives.
				// Do NOT call follow_the_light here — it would fire "Searching for the light."
				// at the delivery location, which is confusing and too far from the beacon.
				console.log('Arduino reports stowed=true. Returning to dock via run_mission.');
			}
		}
