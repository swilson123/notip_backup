...
var get_roll = function (white_rabbit) {
    ...
    if (imu_fresh) return white_rabbit.imu_data.roll * (Math.PI / 180);
    ...
    if (att_fresh) return att.roll;

    // Both stale — return last known. The star still shines.
    if (white_rabbit.imu_data && typeof white_rabbit.imu_data.roll === 'number') {
        return white_rabbit.imu_data.roll * (Math.PI / 180);
    }
    return (att && att.roll) || 0;
};
