var yaw_white_rabbit_for_package_delivery = function (white_rabbit) {
    if (!white_rabbit.mission.finished_package_yaw) {
        ...
        if (!white_rabbit.mission.package_delivery_yaw) {
            // Turn 180° from the arrival heading before delivering. Latched on the first
            // delivery tick so the target stays fixed while Noah spins toward it.
            // `|| 360` keeps the target truthy when the 180° flip lands exactly on 0°
            // (due north) — otherwise the `!package_delivery_yaw` guard above would treat
            // it as unset and recompute a fresh target every tick, so Noah would never
            // converge. 360 and 0 are the same heading, so the alignment math is unaffected.
            white_rabbit.mission.package_delivery_yaw = ((white_rabbit_heading + 180) % 360) || 360;
            console.log("Delivery yaw: 180° from arrival heading = " + white_rabbit.mission.package_delivery_yaw);
        }
        ...
        if (angleDifference <= 5) {
            ...
            // A blind 180° turn does NOT leave Noah facing the first return waypoint (the
            // old bearing-to-prior-waypoint yaw did), so don't tell run_mission he's
            // pre-aligned — let it spin to the return waypoint normally.
            ...
            // Stamp when the delivery command went out so run_mission can time-box the
            // wait for completion and never hang at the drop-off if the Arduino's
            // auto_delivery -> 0 signal never arrives.
            white_rabbit.mission._delivery_sent_at = Date.now();
            if (white_rabbit.mission._auto_delivery_timer) clearTimeout(white_rabbit.mission._auto_delivery_timer);
            white_rabbit.mission._auto_delivery_timer = setTimeout(() => {
