var yaw_white_rabbit_for_package_delivery = function (white_rabbit) {
    if (!white_rabbit.mission.finished_package_yaw) {
        ...
        if (angleDifference <= 10) {
            ...
            // Safety fallback: if Arduino never confirms delivery within 30 s, force return.
            // Prevents Noah from standing at the door forever on a cable fault or firmware hang.
            if (white_rabbit.mission._delivery_confirm_timeout) clearTimeout(white_rabbit.mission._delivery_confirm_timeout);
            white_rabbit.mission._delivery_confirm_timeout = setTimeout(() => {
                white_rabbit.mission._delivery_confirm_timeout = null;
                if (!white_rabbit.mission.package_delivered) {
                    console.log('yaw_delivery: Arduino confirm timeout — forcing package_delivered');
                    if (white_rabbit.voice) white_rabbit.voice.say('Delivery confirmed. Heading home.');
                    white_rabbit.mission.package_delivered = true;
