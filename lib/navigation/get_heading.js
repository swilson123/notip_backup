...
var get_heading = function (white_rabbit) {
    ...
    // Gate on connected (like get_pitch/get_roll) so "IMU enabled" is the trigger:
    // when the IMU is connected and fresh it is the heading source everywhere; the
    // Pixhawk EKF heading is used only when the IMU is disabled or its stream drops.
    var imu_fresh = white_rabbit.imu_data && white_rabbit.imu_data.connected
                    && typeof imu === 'number' && imu_ts && (now - imu_ts) < HEADING_STALE_MS;

    // Fresh primary — BNO055 IMU heading (compass_offset_deg already applied).
    if (imu_fresh) return imu;

    // Fresh fallback — Pixhawk EKF heading (pixhawk_offset_deg applied).
