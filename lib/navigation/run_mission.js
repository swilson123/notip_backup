...
function maybe_disable_sidewalk_follow_on_breadcrumb_return(white_rabbit, nav_tuning) {
    ...
    if (gate_waypoint) {
        // Use the current breadcrumb TARGET position rather than Noah's noisy GPS.
        // The breadcrumb path was recorded during outbound travel, so it has a
        // point within breadcrumb_sample_distance_m of the gate. When that breadcrumb
        // point is the current target, Noah is heading directly through the gate —
        // not 3m before it. Fall back to Noah's own position if no breadcrumb target.
        ...
        if (Array.isArray(white_rabbit.mission.breadcrumb_path) && typeof crumb_idx === 'number') {
            let crumb_target = white_rabbit.mission.breadcrumb_path[crumb_idx];
            if (crumb_target && crumb_target.lat && crumb_target.lng) {
                check_lat = crumb_target.lat;
                check_lng = crumb_target.lng;
            }
        }
