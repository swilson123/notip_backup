    ...
    if (rover.robot_data.is_armed) {
        if (rover.mission.path_clear) {
            ...
            if (waypoint.latitude && waypoint.longitude) {
                ...
                if (Math.abs(rover.robot_data.yaw_to_waypoint) > 25 || distance_to_waypoint_meters < 1 && Math.abs(rover.robot_data.yaw_to_waypoint) > 5) {
                    ...
                    if (rover.motor.current_steering_type == "four_wheels") {
                        ...
                        var steering_and_rpm = rover.calc_steering_and_rpm(rover, rover.robot_data.yaw_to_waypoint, motor_speed_cmd);
                        ...
                        setTimeout(() => {
                            //front passenger
                            rover.move_rover(rover, 1, steering_and_rpm.motor_rpm.front_passenger, "run_mission");
                        }, 10);

                        setTimeout(() => {
                            //rear passenger side
                            rover.move_rover(rover, 2, steering_and_rpm.motor_rpm.back_passenger, "run_mission");

                        }, 20);

                        setTimeout(() => {
                            //front driver side
                            rover.move_rover(rover, 3, steering_and_rpm.motor_rpm.front_driver, "run_mission");
                        }, 30);

                        setTimeout(() => {
                            //rear driver side
                            rover.move_rover(rover, 4, steering_and_rpm.motor_rpm.back_driver, "run_mission");
                        }, 40);
        ...
        console.log("Rover is disarmed. Mission paused.");
    };
