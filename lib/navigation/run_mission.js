...
// Hand the return-to-dock sequence off to dock_white_rabbit (which engages
// follow_the_light once the beacon is fresh). Resets dock state and starts the
// 250 ms dock loop if it isn't already running.
function start_dock_approach(white_rabbit) {
...
// True once all four steering servos have ramped back to straight (~1500 PWM)
// from spin geometry (~1750). Uses set_pwm, the ramped position the servo driver
// tracks toward the commanded value (same readiness proxy yaw_white_rabbit uses).
function steering_is_straight(white_rabbit) {
    const near = function (s) { return s && Math.abs(s.set_pwm - 1500) <= 25; };
    return near(white_rabbit.servos.motor_front_driver)
        && near(white_rabbit.servos.motor_back_driver)
        && near(white_rabbit.servos.motor_front_passenger)
        && near(white_rabbit.servos.motor_back_passenger);
}
            ...
            if (!white_rabbit.mission.pause_mission) {
                ...
                if (white_rabbit.mission.dock_return_phase) {
                    if (white_rabbit.mission.dock_return_phase === 'align_heading') {
                        ...
                        // Shared handoff out of alignment. Stop the spin, then move to the
                        // straighten_wheels phase — the yaw search left the steering servos in
                        // spin geometry (~1750), and the wheels must be squared up to straight
                        // before backing up onto the dock, or Noah reverses while crabbed.
                        const _begin_dock_handoff = function (reason) {
                            ...
                            white_rabbit.mission.dock_return_phase = 'straighten_wheels';
                            white_rabbit.mission.dock_straighten_start_ts = Date.now();
                            white_rabbit.logs.run_mission.log(white_rabbit, 'dock return: ' + reason + ' — straightening wheels before approach');
                            if (white_rabbit.voice) white_rabbit.voice.say('Light found. Straightening up.');
                        };

                        if (_light_found) {
                            _begin_dock_handoff('beacon acquired (angle_x='
                                + (white_rabbit.irlock.target ? white_rabbit.irlock.target.angle_x.toFixed(1) : '?') + '°)');
                            return;
                        }
                        ...
                        if (_undock_hdg === null) {
                            ...
                            if (_align_elapsed > 30000) {
                                // Straighten before the blind approach too — we may have
                                // timed out mid-yaw with the wheels still in spin geometry.
                                _begin_dock_handoff('heading alignment timeout (30s)');
                            } else {
                    ...
                    if (white_rabbit.mission.dock_return_phase === 'straighten_wheels') {
                        // Square all four wheels to straight (1500) and hold the motors
                        // stopped until the steering servos have actually ramped there.
                        // follow_the_light only steers the rear pair (12/14) during the
                        // reverse approach, so the front pair (11/13) must be straightened
                        // here or Noah backs onto the dock crabbed.
                        ...
                        white_rabbit.move_white_rabbit(white_rabbit, 1, 0, 'dock_straighten');
                        white_rabbit.move_white_rabbit(white_rabbit, 2, 0, 'dock_straighten');
                        white_rabbit.move_white_rabbit(white_rabbit, 3, 0, 'dock_straighten');
                        white_rabbit.move_white_rabbit(white_rabbit, 4, 0, 'dock_straighten');

                        // Timeout so a stuck/slow servo can't stall the dock forever.
                        const _straighten_timed_out = white_rabbit.mission.dock_straighten_start_ts
                            && (Date.now() - white_rabbit.mission.dock_straighten_start_ts) > 5000;

                        if (steering_is_straight(white_rabbit) || _straighten_timed_out) {
                            white_rabbit.mission.dock_straighten_start_ts = null;
                            if (white_rabbit.dock.manual_dock_required && white_rabbit.dock.rc_dock >= 1100) {
                                ...
                                white_rabbit.logs.run_mission.log(white_rabbit, 'dock return: wheels straight — waiting for RC dock command');
                                if (white_rabbit.voice) white_rabbit.voice.say('Waiting for dock command.');
                            } else {
                                white_rabbit.mission.dock_return_phase = 'docking';
                                white_rabbit.logs.run_mission.log(white_rabbit, 'dock return: wheels straight — starting dock_white_rabbit');
                                if (white_rabbit.voice) white_rabbit.voice.say('Looking for the light.');
                                start_dock_approach(white_rabbit);
                            }
                        }
                        return;
                    }
