...
function maybe_disable_sidewalk_follow_on_breadcrumb_return(white_rabbit, nav_tuning) {
    ...
    // Return gate: if we pass back through the gate waypoint (sharp turn OR close
    // pair), turn edge-follow off.
    let gate_waypoint = null;
    if (Array.isArray(white_rabbit.mission.waypoints)) {
        for (let i = 0; i < white_rabbit.mission.waypoints.length; i++) {
            let wp = white_rabbit.mission.waypoints[i];
            if (!wp || !wp.lat || !wp.lng) continue;
            if (is_sidewalk_gate_waypoint(white_rabbit, wp.seq, nav_tuning)) {
                gate_waypoint = wp;
                break;
            }
        }
    ...
    if (gate_waypoint) {
        // Measure from the breadcrumb point Noah has just REACHED, not the upcoming
        // target. breadcrumb_return_index points at the NEXT target, so checking it
        // tripped the gate one breadcrumb early — while Noah was still short of the gate
        // (heading toward it). The just-reached point (index + 1) puts the OFF right when
        // he arrives AT the gate. Fall back to the target, then Noah's own GPS.
        let check_lat = lat, check_lng = lng;
        let crumb_idx = white_rabbit.mission.breadcrumb_return_index;
        if (Array.isArray(white_rabbit.mission.breadcrumb_path) && typeof crumb_idx === 'number') {
            let reached = white_rabbit.mission.breadcrumb_path[crumb_idx + 1]
                || white_rabbit.mission.breadcrumb_path[crumb_idx];
            if (reached && reached.lat && reached.lng) {
                check_lat = reached.lat;
                check_lng = reached.lng;
            }
        }
        let gate_dist_m = white_rabbit.gps_distance(check_lat, check_lng, gate_waypoint.lat, gate_waypoint.lng) * 1000;
        if (gate_dist_m <= 1.0) {
            white_rabbit.mission.sidewalk_follow_active = false;
            white_rabbit.logs.run_mission.log(white_rabbit, 'sidewalk gate: returned near gate waypoint ' + gate_waypoint.seq + ' (crumb target ' + gate_dist_m.toFixed(2) + 'm from gate) -- sidewalk following OFF');
            if (white_rabbit.voice) white_rabbit.voice.say_event('sidewalk_off');
        }
    }
}
        ...
        if (white_rabbit.mission.path_clear) {
            ...
            if (!white_rabbit.mission.pause_mission) {
                ...
                if (white_rabbit.mission.package_delivered) {
                    // On the first tick of breadcrumb return, reset the edge trail.
                    // After the delivery U-turn the camera faces the opposite direction,
                    // so outbound x-values are mirrored. Without a reset, every return
                    // frame fails the consistency gate (x jump ≈ full sidewalk width)
                    // and the trail stays empty for the entire return leg.
                    if (!white_rabbit.mission.breadcrumb_return_trail_reset) {
                        white_rabbit.mission.breadcrumb_return_trail_reset = true;
                        ...
                        if (is_sidewalk_gate_waypoint(white_rabbit, white_rabbit.mission.current_mission_seq, nav_tuning)) {
                            if (!white_rabbit.mission.package_delivered) {
                                if (!white_rabbit.mission.sidewalk_follow_active) {
                                    ...
                                    if (white_rabbit.edge_trail) white_rabbit.edge_trail.start();
                                    white_rabbit.logs.run_mission.log(white_rabbit, 'sidewalk gate: marker at waypoint ' + white_rabbit.mission.current_mission_seq + ' — sidewalk following ON');
                        ...
                        if (white_rabbit.motor.current_steering_type == "four_wheels") {
                            ...
                            if (Math.abs(white_rabbit.robot_data.yaw_to_waypoint) > 90 && !follow_sidewalk_enabled(white_rabbit)) {
                                ...
                                let _edge_max_forward_m = (_vis_cfg && typeof _vis_cfg.edge_correction_max_forward_m === 'number')
                                    ...
                                    : 1.0;
                                _sw_conf = Math.max(0.4, Math.min(1.0, _sw_conf));
                                ...
                                let vision_fresh = (_follow_sw && _det && typeof _det.x_angle_deg === 'number'
                                    ...
                                    && !_edge_out_of_lane);
                                _nc.sidewalk_vision_fresh = vision_fresh;
                                if (vision_fresh) {
                                ...
                                // Roll-tilt recovery: detect a sudden roll spike (wheel off sidewalk edge)
                                // and steer back in the opposite direction.
                                // A slow EMA tracks baseline roll so cross-slope sidewalks don't trigger.
                                // Positive roll = right side lower → steer left (negative correction).
                                // Negative roll = left side lower  → steer right (positive correction).
                                const _roll_rad = get_roll(white_rabbit);
                                const _roll_deg = _roll_rad * (180 / Math.PI);
                                if (typeof _nc.roll_baseline_deg !== 'number') _nc.roll_baseline_deg = _roll_deg;
                                _nc.roll_baseline_deg = 0.05 * _roll_deg + 0.95 * _nc.roll_baseline_deg;
                                const _roll_dev = _roll_deg - _nc.roll_baseline_deg;
                                const _roll_threshold = (_vis_cfg && typeof _vis_cfg.roll_tilt_threshold_deg === 'number') ? _vis_cfg.roll_tilt_threshold_deg : 3.0;
                                const _roll_gain      = (_vis_cfg && typeof _vis_cfg.roll_tilt_gain          === 'number') ? _vis_cfg.roll_tilt_gain          : 2.0;
                                const _roll_max       = (_vis_cfg && typeof _vis_cfg.roll_tilt_max_deg       === 'number') ? _vis_cfg.roll_tilt_max_deg       : 10.0;
                                const roll_correction_deg = (_follow_sw && Math.abs(_roll_dev) > _roll_threshold)
                                    ? clamp(-_roll_dev * _roll_gain, -_roll_max, _roll_max)
                                    : 0;

                                // LiDAR diagonal edge warning: zone 1 (30-60° right-forward) and
                                // zone 10 (300-330° left-forward). At 33° tilt these beams hit the
                                // ground diagonally ahead-right / ahead-left. A sudden deviation from
                                // the rolling baseline signals a curb or drop-off — steer away.
                                // Both sides trigger simultaneously → no correction (symmetric obstacle).
                                const _lz_right = Array.isArray(white_rabbit.zones) ? white_rabbit.zones.find(z => z.zone === 1)  : null;
                                const _lz_left  = Array.isArray(white_rabbit.zones) ? white_rabbit.zones.find(z => z.zone === 10) : null;
                                const _lz_stale = 1500;
                                const _lz_ok = z => z && z.timestamp && (_now - z.timestamp) < _lz_stale && typeof z.distance_mm === 'number' && z.distance_mm > 0;
                                const _lz_warn_mm = (_vis_cfg && typeof _vis_cfg.lidar_edge_warn_mm === 'number') ? _vis_cfg.lidar_edge_warn_mm : 300;
                                const _lz_gain    = (_vis_cfg && typeof _vis_cfg.lidar_edge_gain    === 'number') ? _vis_cfg.lidar_edge_gain    : 8.0;
                                const _lz_max_deg = (_vis_cfg && typeof _vis_cfg.lidar_edge_max_deg === 'number') ? _vis_cfg.lidar_edge_max_deg : 8.0;

                                if (typeof _nc.lidar_base_right !== 'number') _nc.lidar_base_right = _lz_ok(_lz_right) ? _lz_right.distance_mm : null;
                                if (typeof _nc.lidar_base_left  !== 'number') _nc.lidar_base_left  = _lz_ok(_lz_left)  ? _lz_left.distance_mm  : null;
                                if (_follow_sw && _lz_ok(_lz_right))
                                    _nc.lidar_base_right = _nc.lidar_base_right === null ? _lz_right.distance_mm : 0.1 * _lz_right.distance_mm + 0.9 * _nc.lidar_base_right;
                                if (_follow_sw && _lz_ok(_lz_left))
                                    _nc.lidar_base_left  = _nc.lidar_base_left  === null ? _lz_left.distance_mm  : 0.1 * _lz_left.distance_mm  + 0.9 * _nc.lidar_base_left;

                                const _lz_dev_r = (_follow_sw && _lz_ok(_lz_right) && _nc.lidar_base_right !== null) ? Math.abs(_lz_right.distance_mm - _nc.lidar_base_right) : 0;
                                const _lz_dev_l = (_follow_sw && _lz_ok(_lz_left)  && _nc.lidar_base_left  !== null) ? Math.abs(_lz_left.distance_mm  - _nc.lidar_base_left)  : 0;
                                const _lz_r_warn = _lz_dev_r > _lz_warn_mm && _lz_dev_r >= _lz_dev_l;
                                const _lz_l_warn = _lz_dev_l > _lz_warn_mm && _lz_dev_l >  _lz_dev_r;
                                const lidar_correction_deg = _lz_r_warn ? clamp(-(_lz_dev_r / 1000) * _lz_gain, -_lz_max_deg, 0)
                                                           : _lz_l_warn ? clamp( (_lz_dev_l / 1000) * _lz_gain, 0, _lz_max_deg)
                                                           : 0;

                                // When sidewalk-following is active, partially suppress GPS heading so
                                // vision owns lateral correction but GPS still guides through curves.
                                // Full suppression breaks curves: on a smooth curve both edges shift equally
                                // so u_target ≈ 0 and vision gives no steering — GPS waypoints placed
                                // along the curve are what actually steer Noah through the turn.
                                // At full vision authority GPS is at 40%; as vision fades GPS returns fully.
                                let nav_yaw_weight = _follow_sw ? (1 - vision_authority * 0.6) : 1.0;
                                let steering_target_deg = (nav_yaw * effective_steering_gain * nav_yaw_weight) + crosstrack_term + vision_correction_deg + roll_correction_deg + lidar_correction_deg;
                                ...
                                log_carrot_decision(white_rabbit,
                                    ...
                                    + ' | nav_yaw=' + nav_yaw.toFixed(1) + '°(w=' + nav_yaw_weight.toFixed(2) + ')'
                                    ...
                                    + ' roll=' + _roll_dev.toFixed(1) + '°→' + roll_correction_deg.toFixed(1) + '°'
                                    + ' lidar_r=' + _lz_dev_r.toFixed(0) + 'mm lidar_l=' + _lz_dev_l.toFixed(0) + 'mm→' + lidar_correction_deg.toFixed(1) + '°'
