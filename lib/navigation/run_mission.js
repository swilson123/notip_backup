...
function get_adjusted_nav_target(rover, waypoint_lat, waypoint_lng, waypoint_bearing, dist_to_waypoint_m, ha, seeking) {
    ...
    // Use the per-band centerline interpolated at the carrot's forward distance so the
    // carrot tracks the actual sidewalk curve. Fall back to the smoothed single-point
    // offset if the centerline is empty (e.g., model still warming up).
    let lateral_at_carrot_raw = interpolate_centerline_lateral(detection.centerline, carrot_dist_m);
    ...
    let lateral_m = clamp(lateral_at_carrot * correction_direction * -1, -max_lateral_m, max_lateral_m);
