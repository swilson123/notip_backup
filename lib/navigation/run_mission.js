...
var run_mission = function (white_rabbit) {
    if (white_rabbit.robot_data.is_armed) {
        ...
        // Triggered when avoid_object (LiDAR) has been running continuously for
        // avoidance_timeout_ms without clearing — meaning the white_rabbit has been
        // spinning but never found a way through.
        if (!white_rabbit.mission.package_delivered && !white_rabbit.mission.finished_package_yaw) {
            ...
            if (path_blocked) {
                ...
                if (Date.now() - white_rabbit.mission.realsense_blocked_since >= block_timeout) {
                    ...
                    white_rabbit.logs.run_mission.log(white_rabbit, 'Fallback delivery (avoidance timeout): facing waypoint ' + last_seq);
        ...
        if (white_rabbit.mission.path_clear) {
            ...
            if (!white_rabbit.mission.pause_mission) {
                ...
                if (white_rabbit.mission.package_delivered) {
                    ...
                    // was last visible.
                    if (white_rabbit.mission.current_mission_seq == 0
