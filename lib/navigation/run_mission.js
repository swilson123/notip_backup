...
function get_realsense_steering_bias_deg(rover) {
    ...
    let correction = detection.offset_meters * vision.correction_gain_deg_per_meter * vision.correction_direction * detection.confidence;
...
// Returns true when a high-threat RealSense object blocks the rover path.
// The RealSense camera (16 in / 0.406 m) sees low obstacles the 360° lidar (24 in / 0.61 m) may miss.
function check_realsense_object_stop(rover) {
                ...
                if (waypoint.latitude && waypoint.longitude) {
                    ...
                    if (mission_yaw_should_run) {
                        ...
                        if (rover.motor.current_steering_type == "four_wheels") {
                            ...
                            if (check_realsense_object_stop(rover)) {
                                rover.mission.path_clear = false;
                                rover.move_rover(rover, 1, 0, "rs_object_stop");
                                rover.move_rover(rover, 2, 0, "rs_object_stop");
                                rover.move_rover(rover, 3, 0, "rs_object_stop");
                                rover.move_rover(rover, 4, 0, "rs_object_stop");
                                rover.logs.run_mission.log(rover, 'RealSense object in rover path: stopping');
                                return;
                            }
