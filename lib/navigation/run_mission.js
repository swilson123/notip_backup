                    ...
                    if (mission_yaw_should_run) {
                        ...
                        if (white_rabbit.motor.current_steering_type == "four_wheels") {
                            ...
                            if (Math.abs(white_rabbit.robot_data.yaw_to_waypoint) > 25) {
                                ...
                                // When vision is confidently tracking the path, fade out GPS crosstrack —
                                // on curves it points back to the straight GPS line and fights the carrot.
                                let _vis_det_conf = (white_rabbit.realsense && white_rabbit.realsense.path_detection)
                                    ? white_rabbit.realsense.path_detection.confidence : 0;
                                let crosstrack_conf_scale = clamp(1.0 - (_vis_det_conf - 0.6) / 0.25, 0, 1);
                                ...
                                let steering_target_deg = (nav_yaw * effective_steering_gain) + (crosstrack_bias_deg * vision_scale * crosstrack_conf_scale) + path_heading_bias_deg;
