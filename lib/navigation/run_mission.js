...
function get_realsense_steering_bias_deg(rover, seeking) {
                    ...
                    if (mission_yaw_should_run) {
                        ...
                        if (rover.motor.current_steering_type == "four_wheels") {
                            ...
                            if (Math.abs(rover.robot_data.yaw_to_waypoint) > 25) {
                                ...
                                let seek_enter_m = seek_cfg && typeof seek_cfg.sidewalk_seek_offset_m === 'number' ? seek_cfg.sidewalk_seek_offset_m : 0.2;
                                ...
                                if (!rover.mission.nav_control.sidewalk_seeking) {
                                    if (seek_offset_abs > seek_enter_m) rover.mission.nav_control.sidewalk_seeking = true;
                                } else {
                                    if (seek_offset_abs < seek_exit_m) rover.mission.nav_control.sidewalk_seeking = false;
                                }
                                let sidewalk_seeking = rover.mission.nav_control.sidewalk_seeking;

                                let vision_steering_bias_deg = get_realsense_steering_bias_deg(rover, sidewalk_seeking);
                                let crosstrack_bias_deg = get_gps_crosstrack_bias_deg(rover, waypoint.latitude, waypoint.longitude);

                                // Fade vision + crosstrack to zero as rover nears a waypoint turn.
                                // GPS owns the turn; vision resumes on the next straight segment.
                                let base_vision_scale = distance_to_waypoint_meters < 5.0
                                    ? clamp((distance_to_waypoint_meters - 1.5) / 3.5, 0, 1)
                                    : 1.0;
                                let seek_boost = sidewalk_seeking && seek_cfg && typeof seek_cfg.sidewalk_seek_scale === 'number'
                                    ? seek_cfg.sidewalk_seek_scale : 1.0;
                                let vision_scale = base_vision_scale * seek_boost;

                                let steering_target_deg = (yaw_to_waypoint * nav_tuning.two_wheel_steering_gain) + (vision_steering_bias_deg * vision_scale) + (crosstrack_bias_deg * vision_scale);
