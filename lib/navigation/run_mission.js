...
function follow_sidewalk_enabled(white_rabbit) {
    ...
    // nav_tuning.sidewalk_gate_always_open=true bypasses the gate waypoint requirement —
    // useful for testing on the sidewalk without needing to hit a gate waypoint first.
    const nav = white_rabbit.nav_tuning || {};
    if (nav.sidewalk_gate_always_open === true) {
        return true;
    }
    return !!(white_rabbit.mission && white_rabbit.mission.sidewalk_follow_active === true);
}
...
var run_mission = function (white_rabbit) {
    if (white_rabbit.robot_data.is_armed) {
        ...
        // ----- Sidewalk gate: proximity close on return -----
        // Independent of the waypoint-based close above: the gate-open moment records the
        // exact GPS spot where sidewalk_follow_active turned ON. On the return trip, coming
        // back within 1m of that same spot closes the gate too — precise regardless of how
        // closely the route's gate waypoint lines up with where the camera actually started
        // steering. Checked every tick, not just at waypoint arrival.
        if (white_rabbit.mission.package_delivered
                && white_rabbit.mission.sidewalk_follow_active
                && white_rabbit.mission.sidewalk_gate_open_lat
                && white_rabbit.mission.sidewalk_gate_open_lng) {
            let _dist_to_gate_open_m = white_rabbit.gps_distance(
                white_rabbit.robot_data.robot_latitude, white_rabbit.robot_data.robot_longitude,
                white_rabbit.mission.sidewalk_gate_open_lat, white_rabbit.mission.sidewalk_gate_open_lng
            ) * 1000;
            if (_dist_to_gate_open_m < 1.0) {
                ...
                // Snap back to the waypoint recorded when the gate opened rather than trusting
                // wherever the camera-steered stretch left current_mission_seq — see the capture
                // site in is_sidewalk_gate_waypoint's pending-open block above.
                if (white_rabbit.mission.sidewalk_gate_seq) {
                    white_rabbit.mission.current_mission_seq = white_rabbit.mission.sidewalk_gate_seq;
                    white_rabbit.mission.sidewalk_gate_seq = null;
                }
                white_rabbit.logs.run_mission.log(white_rabbit, 'sidewalk gate: within 1m of open position — sidewalk following OFF, back to mission mode, seq=' + white_rabbit.mission.current_mission_seq);
                if (white_rabbit.voice) white_rabbit.voice.say_event('sidewalk_off');
            }
        }
        // ----- end sidewalk gate proximity close -----
                        ...
                        if (is_sidewalk_gate_waypoint(white_rabbit, white_rabbit.mission.current_mission_seq, nav_tuning)) {
                            if (!white_rabbit.mission.package_delivered) {
                                if (!white_rabbit.mission.sidewalk_follow_active && !white_rabbit.mission.sidewalk_gate_pending) {
                                    ...
                                    // Recorded now, before this waypoint-reached tick advances current_mission_seq
                                    // below — the camera-steered stretch that follows can't be trusted to land
                                    // current_mission_seq back on this exact waypoint (it only advances when
                                    // Noah happens to pass within 1m of a mapped point, which a sidewalk-edge
                                    // path doesn't guarantee). Restored verbatim when the gate closes.
                                    white_rabbit.mission.sidewalk_gate_seq = white_rabbit.mission.current_mission_seq;
                        ...
                        if (white_rabbit.motor.current_steering_type == "four_wheels") {
                            ...
                            if (white_rabbit.servos.motor_front_driver.set_pwm > 1400 && white_rabbit.servos.motor_front_driver.set_pwm < 1600 &&
                                ...
                                if (white_rabbit.mission.sidewalk_gate_pending) {
                                    ...
                                    white_rabbit.mission.sidewalk_gate_open_lat = white_rabbit.robot_data.robot_latitude;
                                    white_rabbit.mission.sidewalk_gate_open_lng = white_rabbit.robot_data.robot_longitude;
