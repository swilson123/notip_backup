...
// True when waypoint `seq` is a sidewalk-gate marker. The trigger is a close pair:
// this waypoint sits within nav_tuning.sidewalk_gate_pair_distance_m of an adjacent
// waypoint. Placing two waypoints right next to each other (same arrival bubble) at
// the road↔sidewalk boundary is the gate, with no physical turn required.
            ...
            if (!white_rabbit.mission.pause_mission) {
                ...
                if (waypoint.latitude && waypoint.longitude) {
                    ...
                    if (white_rabbit.mission.nav_accuracy.inside_radius_count >= white_rabbit.mission.nav_accuracy.required_inside_radius_count) {
                        ...
                        // a deliberate close pair of waypoints (see is_sidewalk_gate_waypoint).
                        // Reaching it OUTBOUND turns
                        ...
                        if (white_rabbit.mission.package_delivered
                                && white_rabbit.mission.current_mission_seq <= 1
                                && white_rabbit.dock && white_rabbit.dock.undock_latitude
                                && !white_rabbit.mission.dock_overrun_consumed) {

                            white_rabbit.mission.dock_overrun_consumed = true;

                            const _begin_align_heading = function () {
                                white_rabbit.mission.dock_return_phase = 'align_heading';
                                // Seed the manual-dock policy for this return. When
                                // nav_tuning.manual_dock_required is true, Noah won't auto-dock:
                                // once aligned he parks in await_dock_command (default rc_dock
                                // 1500 >= 1100 keeps him waiting) until an explicit trigger — the
                                // RC dock command OR the hook limit switch — starts follow_the_light.
                                white_rabbit.dock.manual_dock_required = white_rabbit.nav_tuning.manual_dock_required;
                            ...
                            // The dock arrival radius (and the overshoot-capture debounce above)
                            // can latch "reached" while GPS bearing is still noisy this close in
                            // — leaving Noah short of the real dock. Drive the configured overrun
                            // distance the rest of the way using the same closed-loop odometry
                            // nudge the voice "move forward" command uses (dead-reckoning, not
                            // another GPS check) before aligning to dock.
                            const _overrun_m = typeof nav_tuning.dock_arrival_overrun_m === 'number' ? nav_tuning.dock_arrival_overrun_m : 1.0;
                            if (_overrun_m > 0 && white_rabbit.voice && typeof white_rabbit.voice.nudge_forward === 'function') {
                                white_rabbit.logs.run_mission.log(white_rabbit, 'dock return: at undock position — driving ' + _overrun_m.toFixed(2) + 'm further before aligning');
                                white_rabbit.voice.nudge_forward(_overrun_m, _begin_align_heading, 'dock_return overrun');
                            } else {
