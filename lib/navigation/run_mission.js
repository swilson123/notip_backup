...
function get_adjusted_nav_target(rover, waypoint_lat, waypoint_lng, waypoint_bearing, dist_to_waypoint_m, ha, seeking) {
    ...
    // The fused version persists across rejected/low-confidence frames and gives
    // a tangent we can use to anticipate turns.
    let fused_centerline = null;
    let path_map = rover.realsense && rover.realsense.path_map;
    let rover_heading_for_map = path_map_lib.get_rover_heading(rover);
    if (path_map && path_map.points && path_map.points.length > 0 && typeof rover_heading_for_map === 'number') {
    ...
    // Project the carrot ahead of the rover along the waypoint bearing, then shift it
    // perpendicular onto the sidewalk centerline.
    let rover_lat = rover.robot_data.robot_latitude;
    let rover_lng = rover.robot_data.robot_longitude;
    let carrot = destination_from_point(rover_lat, rover_lng, waypoint_bearing, carrot_dist_m);
        ...
        if (rover.mission.path_clear) {
            ...
            if (!rover.mission.pause_mission) {
                ...
                if (waypoint.latitude && waypoint.longitude) {
                    ...
                    let target_speed_cmd = Math.max(35, Math.round(200 * Math.min(yaw_speed_scale, distance_speed_scale, vision_speed_scale)));
                        ...
                        if (rover.motor.current_steering_type == "four_wheels") {
                            ...
                            if (Math.abs(rover.robot_data.yaw_to_waypoint) > 25) {
                                ...
                                if (_seek_path_map && _seek_path_map.points && _seek_path_map.points.length > 0 && typeof _seek_heading === 'number') {
                                    ...
                                    let _seek_fused = path_map_lib.get_fused_centerline(
                                        _seek_path_map,
                                        rover.robot_data.robot_latitude,
                                        rover.robot_data.robot_longitude,
                                        _seek_heading,
                                    ...
                                    let _near_h_rad = path_map_lib.get_near_field_heading(_seek_fused, 1.5);
                                ...
                                if (!rover.mission.nav_control.sidewalk_seeking) {
                                    if (seek_offset_abs > seek_enter_m || history_analysis.sustained_seeking || heading_diverged) {
                                        rover.mission.nav_control.sidewalk_seeking = true;
                                    }
                                } else {
                                    if (seek_offset_abs < seek_exit_m && !history_analysis.sustained_seeking && heading_aligned) {
                                        rover.mission.nav_control.sidewalk_seeking = false;
                                    }
                                }
                                ...
                                let steering_target_deg = (nav_yaw * nav_tuning.two_wheel_steering_gain) + (crosstrack_bias_deg * vision_scale) + path_heading_bias_deg;
                                ...
                                if (_det && typeof _det._carrot_dist_m === 'number') {
                                    ...
                                    _carrot_info += ' carrot_dist=' + _det._carrot_dist_m.toFixed(2) + 'm'
                                        ...
                                        + ' crosstrack=' + crosstrack_bias_deg.toFixed(1) + '° vision_scale=' + vision_scale.toFixed(2)
