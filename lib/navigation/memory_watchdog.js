// Memory Watchdog — uses rover.memory to catch failure modes that no single
...
//        by setting rover.mission.avoidance_timed_out so the existing
...
//        > 4° → widen the steering deadband and ease the gain so the rover
//        settles instead of wobbling.
//
// Each watchdog is independent; any can be disabled via setup.json. None ever
// throws — a bad watchdog must not crash the rover.
...
function config_for(rover) {
    return rover.memory_watchdog_config || {
        enabled:                 true,
...
function init_state(rover) {
    if (!rover.mission.memory_watchdog) {
        rover.mission.memory_watchdog = {
    ...
    return rover.mission.memory_watchdog;
}

// ---------- A: stuck detector + recovery ----------

function stuck_eligible(rover) {
    if (!rover.robot_data.is_armed)                                                 return false;
    if (rover.mission.pause_mission)                                                return false;
    if (!rover.mission.path_clear)                                                  return false;
    if (rover.mission.realsense_blocked_since)                                      return false;
    if (rover.mission.avoidance_timed_out)                                          return false;
    if (rover.motor.current_steering_type !== 'two_wheels')                         return false;
    if (rover.mission.nav_control && rover.mission.nav_control.mission_yaw_active)  return false;
    return true;
}

function detect_stuck(rover) {
    if (!rover.memory) return null;
    const r = rover.memory.reflect(STUCK.window_ms);
...
function command_stop(rover) {
    rover.move_rover(rover, 1, 0, 'memory_watchdog recovery stop');
    rover.move_rover(rover, 2, 0, 'memory_watchdog recovery stop');
    rover.move_rover(rover, 3, 0, 'memory_watchdog recovery stop');
    rover.move_rover(rover, 4, 0, 'memory_watchdog recovery stop');
}

function command_reverse(rover, speed) {
    // Straighten the steering — reversing with the wheels turned is unpredictable.
    const zero_pwm = rover.angle_to_pwm(0);
    rover.servo_send_command(rover, 11, zero_pwm.servo1, false);
    rover.servo_send_command(rover, 13, zero_pwm.servo2, false);
    ...
    rover.move_rover(rover, 1, speed,      'memory_watchdog reverse');
    rover.move_rover(rover, 4, speed * -1, 'memory_watchdog reverse');
    rover.move_rover(rover, 3, speed * -1, 'memory_watchdog reverse');
    rover.move_rover(rover, 2, speed,      'memory_watchdog reverse');
}

function reverse_speed_for(rover) {
    return Math.max(40, Math.round((rover.motor.throttle_percentage || 0.25) * 200));
}
...
function check(rover) {
    if (!rover.memory) return false;
    const cfg = config_for(rover);
    if (!cfg.enabled || !cfg.stuck_recovery_enabled) return false;

    const s   = init_state(rover);
    const now = Date.now();
    ...
    if (s.recovery_state === 'reversing') {
        if (now - s.recovery_start_ts >= STUCK.recovery_reverse_ms) {
            ...
            command_stop(rover);
            safe_log(rover, 'memory_watchdog: reverse complete, pausing');
        } else {
            command_reverse(rover, reverse_speed_for(rover));
        }
        return true;
    }
    if (s.recovery_state === 'pause_post_reverse') {
        ...
        if (now - s.recovery_pause_start_ts >= STUCK.recovery_pause_ms) {
            ...
            safe_log(rover, 'memory_watchdog: recovery complete, resuming mission');
        }
        return true;
    }

    // 2. Detect a new stuck event.
    if (!stuck_eligible(rover)) return false;
    const stuck_info = detect_stuck(rover);
    ...
    if (s.recovery_count >= STUCK.max_recoveries) {
        rover.mission.avoidance_timed_out = true;
        s.recovery_count = 0;
        safe_log(rover, 'memory_watchdog: max recoveries (' + STUCK.max_recoveries
    ...
    s.recovery_count   += 1;
    command_stop(rover);
    safe_log(rover, 'memory_watchdog: stuck detected (moved='
    ...
    // and flag any zones the rover is already inside so they don't decay.
    if (rover.learning && typeof rover.learning.add === 'function') {
        ...
        rover.learning.mark_stuck_in_zones(lat, lng);
        rover.learning.add('stuck_event', {
            lat:           lat,
            lng:           lng,
            mission_seq:   rover.mission.current_mission_seq,
            moved_m:       stuck_info.moved_m,
            avg_speed_cmd: stuck_info.avg_speed_cmd,
            span_ms:       stuck_info.span_ms
        });
    }
    if (rover.intelligence) rover.intelligence.consider('stuck_detected');
    return true;
}

// ---------- B: vision-trend pre-slowdown ----------

function get_speed_multiplier(rover) {
    if (!rover.memory) return 1.0;
    const cfg = config_for(rover);
    ...
    const latest = rover.memory.latest();
    ...
    const trend = rover.memory.delta('realsense.confidence', VISION_TREND.window_ms);
    ...
    if (now - s.last_vision_warn_ts > 2000) {
        ...
        safe_log(rover, 'memory_watchdog: vision trending down (conf='
        ...
        if (rover.learning && typeof rover.learning.add === 'function') {
            rover.learning.add('vision_loss', {
...
function get_steering_adjustments(rover) {
    const passthrough = { deadband_boost_deg: 0, gain_multiplier: 1.0 };
    if (!rover.memory) return passthrough;
    const cfg = config_for(rover);
    if (!cfg.enabled || !cfg.yaw_oscillation_enabled) return passthrough;

    const snaps = rover.memory.recent(YAW_OSC.window_ms);
    ...
    if (sign_changes >= YAW_OSC.min_sign_changes && avg_amp > YAW_OSC.min_amplitude_deg) {
        ...
        if (now - s.last_osc_log_ts > 2000) {
            ...
            safe_log(rover, 'memory_watchdog: yaw oscillation (' + sign_changes
            ...
            if (rover.learning && typeof rover.learning.add === 'function') {
                rover.learning.add('yaw_oscillation', {
                    lat:          rover.robot_data && rover.robot_data.robot_latitude,
                    lng:          rover.robot_data && rover.robot_data.robot_longitude,
...
module.exports = {
    ...
    get_steering_adjustments: get_steering_adjustments
};
