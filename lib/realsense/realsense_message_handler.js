...
var realsense_message_handler = function (rover, data) {
	...
	try {
		...
		if (!rover.realsense.json_received) {
			rover.realsense.json_received = true;
	...
	if (parsed_message.message_type === 'path_detection') {
		...
		if (!rover.realsense.path_detection_history) {
			rover.realsense.path_detection_history = [];
		}
		let history_window_ms = (rover.realsense.vision && typeof rover.realsense.vision.detection_history_ms === 'number')
			? rover.realsense.vision.detection_history_ms
			: DEFAULT_DETECTION_HISTORY_MS;
		rover.realsense.path_detection_history.push({
		...
		while (rover.realsense.path_detection_history.length > 0 &&
		       rover.realsense.path_detection_history[0].timestamp < cutoff) {
			rover.realsense.path_detection_history.shift();
		}

		// World-frame sidewalk map: persist centerline observations across frames
		// so the rover can anticipate turns and ride through transient bad frames.
		...
		let path_map = rover.realsense.path_map;
		if (path_map) {
			let vis = rover.realsense.vision || {};
			let rover_lat = rover.robot_data.robot_latitude;
			let rover_lng = rover.robot_data.robot_longitude;
			let heading_deg = path_map_lib.get_rover_heading(rover);
			let eph_m = (rover.robot_data.GPS_RAW_INT && typeof rover.robot_data.GPS_RAW_INT.eph === 'number')
				? rover.robot_data.GPS_RAW_INT.eph / 100 : 1.0;
			let pose_valid = typeof rover_lat === 'number' && typeof rover_lng === 'number'
			...
			if (pose_valid) {
				...
				let is_jump = path_map_lib.detect_gps_jump(path_map, rover_lat, rover_lng, eph_m, jump_floor, jump_mult);

				if (is_jump) {
					path_map.last_jump_ts = now_ts;
				} else {
					path_map.last_pose = { lat: rover_lat, lng: rover_lng, ts: now_ts };
					if (detection.confidence > 0) {
						...
						path_map_lib.ingest_centerline(
							...
							rover_lat, rover_lng, heading_deg, now_ts, merge_r
						);
					}
				}
				...
				path_map_lib.prune(path_map, rover_lat, rover_lng, heading_deg, max_age_ms, max_behind, max_points);
			}
			...
			if (!path_map.last_log_ts || now_ts - path_map.last_log_ts >= 1000) {
				...
				rover.logs.realsense_message_handler.log(rover,
		...
		if (!rover.realsense.last_detection_log_ts || Date.now() - rover.realsense.last_detection_log_ts >= 1000) {
			...
			rover.logs.realsense_message_handler.log(rover, 'RAW: ' + JSON.stringify(parsed_message));
		}
