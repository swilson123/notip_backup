...
function update_vision_zones(white_rabbit) {
	...
	white_rabbit.realsense.objects.forEach(function (obj) {
		...
		let obj_light = 'green';
		if (obj.in_white_rabbit_path && obj.threat_level === 'high' && obj.distance_m <= stop_dist) {
			obj_light = 'red';
...
var realsense_message_handler = function (white_rabbit, data) {
	...
	if (parsed_message.message_type === 'path_detection') {
		...
		let raw_offset = to_number(parsed_message.offset_meters, 0);
		detection.offset_meters = (typeof detection.offset_meters === 'number')
			? 0.4 * raw_offset + 0.6 * detection.offset_meters
			: raw_offset;
