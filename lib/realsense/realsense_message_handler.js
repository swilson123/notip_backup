...
var realsense_message_handler = function (white_rabbit, data) {
	...
	if (parsed_message.message_type === 'path_detection') {
		...
		if (!white_rabbit.realsense.last_detection_log_ts || Date.now() - white_rabbit.realsense.last_detection_log_ts >= 1000) {
			...
			let guide_summary = 'none';
			if (detection.edge_guidance_valid) {
				let fmt_e = function (m, c) { return (m === null ? '--' : m.toFixed(2)) + 'm@' + c.toFixed(2); };
				guide_summary = 'use=' + detection.edge_used
					+ ' L=' + fmt_e(detection.edge_left_m, detection.edge_left_conf)
					+ ' R=' + fmt_e(detection.edge_right_m, detection.edge_right_conf)
