...
function update_vision_zones(rover) {
	...
	VISION_ZONE_NUMBERS.forEach(function (z) {
		...
		if (raw === 'green') {
			...
			// Threat present — start or continue the confirmation timer
			if (vz.first_seen_ts === null) {
				vz.first_seen_ts = now;
			}
			vz.last_raw_threat = raw;
			// Only promote to active light after the object has been visible for 1 second
			if ((now - vz.first_seen_ts) >= VISION_CONFIRM_MS) {
				vz.light = raw;
			}
			// Before confirmation window expires, light stays 'green'
		}
	});
}
