...
// Was realsense_vision.object_emergency_stop_m in setup.json before that section was
// trimmed down to only what the camera connection itself needs; baked in at its
// last-configured value. See CLAUDE.md's realsense_vision note.
var OBJECT_EMERGENCY_STOP_M = 1.0;

function update_vision_zones(white_rabbit) {
	let vision_zones = ensure_vision_zones(white_rabbit);
	let stop_dist = OBJECT_EMERGENCY_STOP_M;
	let now = Date.now();
...
var realsense_message_handler = function (white_rabbit, data) {
	...
	if (parsed_message.message_type === 'path_detection') {
		...
		// Full near-to-far per-band trace (el1..el6 / er1..er6), not just the single
		// closest band per side above -- lets a turn approach be read directly.
		detection.edge_left_bands = Array.isArray(parsed_message.edge_left_bands) ? parsed_message.edge_left_bands : [];
		detection.edge_right_bands = Array.isArray(parsed_message.edge_right_bands) ? parsed_message.edge_right_bands : [];
		...
		let history_window_ms = DEFAULT_DETECTION_HISTORY_MS;
		...
		if (path_map) {
			// gps_jump_floor_m / gps_jump_speed_multiplier / path_map_* were
			// realsense_vision.* setup.json params before that section was trimmed
			// down to only what the camera connection itself needs; baked in here at
			// their last-configured (or already-default) values. edge_lookahead_m is
			// still a live kept param, read normally. See CLAUDE.md's realsense_vision note.
			let GPS_JUMP_FLOOR_M = 0.5;
			let GPS_JUMP_SPEED_MULTIPLIER = 3.0;
			let PATH_MAP_MERGE_RADIUS_M = 0.25;
			let PATH_MAP_MAX_AGE_S = 5.0;
			let PATH_MAP_MAX_BEHIND_M = 2.0;
			let PATH_MAP_MAX_POINTS = 120;
			let PATH_MAP_BIN_WIDTH_M = 0.5;
			...
			if (pose_valid) {
				let is_jump = path_map_lib.detect_gps_jump(path_map, white_rabbit_lat, white_rabbit_lng, eph_m, GPS_JUMP_FLOOR_M, GPS_JUMP_SPEED_MULTIPLIER);
				...
				if (is_jump) {
					...
					if (detection.confidence > 0) {
						path_map_lib.ingest_centerline(
							...
							white_rabbit_lat, white_rabbit_lng, heading_deg, now_ts, PATH_MAP_MERGE_RADIUS_M
						);
					}
				}

				let max_age_ms  = PATH_MAP_MAX_AGE_S * 1000;
				path_map_lib.prune(path_map, white_rabbit_lat, white_rabbit_lng, heading_deg, max_age_ms, PATH_MAP_MAX_BEHIND_M, PATH_MAP_MAX_POINTS);

				// Reproject the persistent map into Noah's CURRENT heading/position and read off
				// the sidewalk center at the same lookahead distance x_angle_deg targets. This is
				// the fallback steering source for when the live camera frame has panned off the
				// sidewalk (e.g. mid-turn) and edge_guidance_valid goes false — the map still knows
				// where the sidewalk continues because it was anchored with compass heading, not
				// the camera's current bore direction.
				let vis = white_rabbit.realsense.vision || {};
				let lookahead_m = typeof vis.edge_lookahead_m === 'number' ? vis.edge_lookahead_m : 0.6096;
				let bin_width_m = PATH_MAP_BIN_WIDTH_M;
				let fused_centerline = path_map_lib.get_fused_centerline(
					path_map, white_rabbit_lat, white_rabbit_lng, heading_deg, bin_width_m, lookahead_m + bin_width_m * 2
				);
				let map_point = null;
				for (let p of fused_centerline) {
					if (!map_point || Math.abs(p.forward_m - lookahead_m) < Math.abs(map_point.forward_m - lookahead_m)) {
						map_point = p;
					}
				}
				detection.map_angle_deg = map_point
					? Math.round(Math.atan2(-map_point.lateral_offset_m, map_point.forward_m) * (180 / Math.PI) * 100) / 100
					: null;
				detection.map_confidence = map_point ? Math.round(map_point.confidence * 100) / 100 : 0;
			}
