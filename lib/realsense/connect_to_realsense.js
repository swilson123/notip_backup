...
function reset_realsense_connection(white_rabbit) {
	...
	white_rabbit.realsense.pitch_stream_ok = false; // depth corrections suspended until stream recovers
...
function start_pitch_stream(white_rabbit) {
	...
	// Explicit stdin error handler — catches broken-pipe before the close event fires.
	// Without this, errors surface as unhandled exceptions and can crash the process.
	if (white_rabbit.realsense.process && white_rabbit.realsense.process.stdin) {
		white_rabbit.realsense.process.stdin.on('error', function (e) {
			white_rabbit.realsense.pitch_stream_ok = false;
			white_rabbit.logs.realsense_message_handler.log(white_rabbit,
				'pitch stream stdin error — depth corrections suspended: ' + e.message);
		});
	}
	...
	white_rabbit.realsense.pitch_send_interval = setInterval(function () {
		...
		try {
			...
			white_rabbit.realsense.pitch_stream_ok = true; // both writes confirmed live
		} catch (e) {
			white_rabbit.realsense.pitch_stream_ok = false;
			// Rate-limited log — once per 5 s so a flapping stream doesn't flood the log
			var _now = Date.now();
			if (_now - (white_rabbit.realsense._pitch_error_ts || 0) > 5000) {
				white_rabbit.realsense._pitch_error_ts = _now;
				white_rabbit.logs.realsense_message_handler.log(white_rabbit,
					'pitch stream write failed — depth corrections suspended: ' + e.message);
			}
		}
	}, 100); // 10 Hz
}
...
function handle_realsense_stdout(white_rabbit, data) {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		white_rabbit.realsense.connecting = false;
		// First good frame after a restart — process is stable, reset backoff counter.
		if (white_rabbit.realsense._restart_count > 0) {
			white_rabbit.realsense._restart_count = 0;
		}
		white_rabbit.realsense_message_handler(white_rabbit, line);
	}
}

function _launch_vision_process(white_rabbit) {
	...
	let vision_config = {
		...
		// Edge-as-guiding-key: look ~edge_lookahead_m ahead (2 ft), hold edge_side_offset_m (1.5 ft) off the edge
		edge_lookahead_m: typeof vision.edge_lookahead_m === 'number' ? vision.edge_lookahead_m : 0.6096,
		edge_side_offset_m: typeof vision.edge_side_offset_m === 'number' ? vision.edge_side_offset_m : 0.4572,
		edge_guidance_bands: typeof vision.edge_guidance_bands === 'number' ? vision.edge_guidance_bands : 8,
		// Edge-choice hysteresis — stick with the chosen edge while confident; don't flip on small jitter
		edge_hysteresis_keep_conf: typeof vision.edge_hysteresis_keep_conf === 'number' ? vision.edge_hysteresis_keep_conf : 0.6,
		edge_hysteresis_switch_margin: typeof vision.edge_hysteresis_switch_margin === 'number' ? vision.edge_hysteresis_switch_margin : 0.2,
		edge_hysteresis_ttl_ms: typeof vision.edge_hysteresis_ttl_ms === 'number' ? vision.edge_hysteresis_ttl_ms : 3000,
		...
		// TRON-grid ground-plane filter — rejects appearance pixels that aren't flat ground at camera height
		ground_grid_filter_enabled: vision.ground_grid_filter_enabled !== false,
		ground_height_tol_m: typeof vision.ground_height_tol_m === 'number' ? vision.ground_height_tol_m : 0.10,
		ground_grid_cell_m: typeof vision.ground_grid_cell_m === 'number' ? vision.ground_grid_cell_m : 0.25,
		ground_grid_min_samples: typeof vision.ground_grid_min_samples === 'number' ? vision.ground_grid_min_samples : 4,
		ground_grid_nonground_ratio: typeof vision.ground_grid_nonground_ratio === 'number' ? vision.ground_grid_nonground_ratio : 0.5
	};
	...
	white_rabbit.realsense.process.on('close', function (code) {
		...
		reset_realsense_connection(white_rabbit);
		// Exponential backoff: 5 s → 10 s → 20 s → 30 s max.
		// Rapid repeated crashes (camera SDK fail, OOM) back off gracefully
		// instead of hammering the Pi with a spawn every 5 seconds.
		var count = white_rabbit.realsense._restart_count || 0;
		var delay_ms = Math.min(5000 * Math.pow(2, count), 30000);
		white_rabbit.realsense._restart_count = count + 1;
		if (count > 0) {
			white_rabbit.logs.realsense_message_handler.log(white_rabbit,
				'Vision restart #' + (count + 1) + ' in ' + (delay_ms / 1000).toFixed(0) + 's');
		}
		setTimeout(function () {
...
function start_realsense_vision(white_rabbit) {
	if (white_rabbit.realsense.process || white_rabbit.realsense.connecting) return;
	white_rabbit.realsense.connecting = true;
	white_rabbit.realsense.pitch_stream_ok = false;

	// Kill any orphaned realsense_vision.py processes without blocking the event loop.
	// spawnSync would freeze all navigation callbacks for ~500 ms on every restart.
	var killer = spawn('pkill', ['-f', 'realsense_vision.py'], { stdio: 'ignore' });
	var launched = false;
	function _do_launch() {
		if (launched) return;
		launched = true;
		_launch_vision_process(white_rabbit);
	}
	killer.on('close', _do_launch);
	killer.on('error', _do_launch); // pkill unavailable — proceed directly
}
