...
// the Python vision subprocess (sidewalk_vision.py). Pitch/roll rotate the depth
// back-projection into a white_rabbit-horizontal frame for the ground-plane filter in
// concrete_edge_detector.py. Heading is currently unused by sidewalk_vision.py but is
// still streamed in case a future edge-prediction pass wants it, same as the old
// realsense_vision.py's _predict_and_score did.
...
function start_realsense_vision(white_rabbit) {
	...
	// Kill any orphaned sidewalk_vision.py processes from a previous crash or restart
	// so they don't hold the camera and cause "No device connected"
	spawnSync('pkill', ['-f', 'sidewalk_vision.py'], { stdio: 'ignore' });

	let vision = white_rabbit.realsense.vision || {};
	// The Python subprocess reads its config SOLELY from this JSON arg (not setup.json),
	// so forward the entire raw realsense_vision section — every setup.json key reaches
	// the detector and stays in sync. Falls back to the curated object if raw is absent.
