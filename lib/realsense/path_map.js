...
// successive vision frames, prunes by age + distance behind the white_rabbit, and
// projects back to the white_rabbit frame on demand. Lets the white_rabbit anticipate turns
...
function get_white_rabbit_heading(white_rabbit) {
    // Use the canonical heading (IMU when enabled/fresh, else Pixhawk EKF with
    // pixhawk_offset_deg applied). Return null when no source exists so callers
    // can skip rather than map against a fake 0.
...
function white_rabbit_to_world(forward_m, lateral_offset_m, white_rabbit_lat, white_rabbit_lng, heading_deg) {
    ...
    let m_per_deg_lng = M_PER_DEG_LAT * Math.cos(deg_to_rad(white_rabbit_lat));
    return {
        lat: white_rabbit_lat + d_north / M_PER_DEG_LAT,
        lng: white_rabbit_lng + d_east  / m_per_deg_lng
    };
}

function world_to_white_rabbit(lat, lng, white_rabbit_lat, white_rabbit_lng, heading_deg) {
    let m_per_deg_lng = M_PER_DEG_LAT * Math.cos(deg_to_rad(white_rabbit_lat));
    let d_north = (lat - white_rabbit_lat) * M_PER_DEG_LAT;
    let d_east  = (lng - white_rabbit_lng) * m_per_deg_lng;
    let H = deg_to_rad(heading_deg);
    // Inverse of white_rabbit_to_world: rotation by -H applied to (d_north, d_east).
...
// GPS jump detection: the white_rabbit's reported position has moved further from the
...
function detect_gps_jump(path_map, white_rabbit_lat, white_rabbit_lng, eph_m, jump_floor_m, jump_multiplier) {
    let last = path_map.last_pose;
    if (!last) return false;
    let step_m = haversine_m(last.lat, last.lng, white_rabbit_lat, white_rabbit_lng);
    let threshold = Math.max(jump_floor_m, eph_m * jump_multiplier);
    return step_m > threshold;
}

function ingest_centerline(path_map, centerline, confidence, white_rabbit_lat, white_rabbit_lng, heading_deg, ts, merge_radius_m) {
    ...
    for (let p of centerline) {
        ...
        let w = white_rabbit_to_world(p.forward_m, p.lateral_offset_m, white_rabbit_lat, white_rabbit_lng, heading_deg);
        ...
        for (let q of path_map.points) {
            ...
            if (dn * dn + de * de <= merge_radius_sq) {
                ...
                // EMA, not additive: a true feature stays high, but a false positive
                // (shadow, wet patch, parked car) decays as later frames see it lower,
                // instead of locking at 1.0 and ghost-steering on the return pass.
                q.confidence = q.confidence * 0.7 + confidence * 0.3;
                q.observed_at = ts;
                merged = true;
                break;
            }
...
function prune(path_map, white_rabbit_lat, white_rabbit_lng, heading_deg, max_age_ms, max_behind_m, max_points) {
    ...
    path_map.points = path_map.points.filter(p => {
        ...
        let d_north = (p.lat - white_rabbit_lat) * M_PER_DEG_LAT;
        let d_east  = (p.lng - white_rabbit_lng) * m_per_deg_lng;
...
// Project all map points back into the current white_rabbit frame, bin by forward
// distance, and emit a confidence-weighted-median centerline. Sorted near→far.
function get_fused_centerline(path_map, white_rabbit_lat, white_rabbit_lng, heading_deg, bin_width_m, max_forward_m) {
...
// relative to the white_rabbit's current heading. Positive = path turning LEFT.
...
// Used as a "white_rabbit pointed wrong" signal that complements offset-based seeking.
...
module.exports = {
    M_PER_DEG_LAT,
    get_white_rabbit_heading,
    haversine_m,
    white_rabbit_to_world,
    world_to_white_rabbit,
    detect_gps_jump,
    ingest_centerline,
    prune,
