...
//Commands: deliver_package, belt, arm
	...
	if (white_rabbit.realsense.process && white_rabbit.realsense.process.stdin && !white_rabbit.realsense.process.stdin.destroyed) {
		try {
			...
			white_rabbit.realsense.process.stdin.write(JSON.stringify({ message: message, value: String(value) }) + '\n');
		} catch (e) {
			console.log(e);
		...
		try {
			...
			white_rabbit.realsense.serial.write(JSON.stringify({ message: message, value: String(value) }) + '\n');
		} catch (e) {
			console.log(e);
