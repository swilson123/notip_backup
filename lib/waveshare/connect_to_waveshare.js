		...
		rover.zling._feedbackInterval = setInterval(() => {
			...
			const p1 = rover.zling.comName1_connected
				? enqueuePromise(rover.motor._writeQueues, 'motor1', async () => {
					...
					try {
						// Read 6 consecutive registers from REG_L_POS_HI (0x20A7)
						// through REG_R_FEEDBACK (0x20AC). One Modbus transaction
						// gets both cumulative position (signed int32 per wheel)
						// and RPM × 10 feedback for both wheels on this driver.
						const res = await withTimeout(
							rover.motor.motor1_client.readHoldingRegisters(rover.zling.REG_L_POS_HI, 6),
							350,
							'Driver 1 feedback read'
						);
						const leftPos    = toSigned32(res.data[0], res.data[1]);
						const rightPos   = toSigned32(res.data[2], res.data[3]);
						// ZLAC8015D RPM registers store RPM × 10 (0.1 RPM resolution).
						// Verified against the manufacturer-derived rasheeddo Python driver.
						const leftSpeed  = toSigned16(res.data[4]) / 10;
						const rightSpeed = toSigned16(res.data[5]) / 10;
						rover.zling.health.driver1_consecutive_read_errors = 0;
						...
						const data = {
							...
							left_position_pulses: leftPos,
							right_position_pulses: rightPos,
							left_motor_id: 3,
							right_motor_id: 1
						};
			...
			const p2 = rover.zling.comName2_connected
				? enqueuePromise(rover.motor._writeQueues, 'motor2', async () => {
					...
					try {
						// Read 6 consecutive registers from REG_L_POS_HI (0x20A7)
						// through REG_R_FEEDBACK (0x20AC). See driver 1 path above
						// for the layout.
						const res = await withTimeout(
