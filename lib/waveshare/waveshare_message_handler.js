...
var waveshare_message_handler = function (rover, data) {
	...
	// Cache per-wheel RPM feedback on rover state so consumers (voice nudges,
	// odometry-based motion, stall detection) can read it without re-parsing
	// the Modbus frame. Driver 1 carries motors 1 & 3, driver 2 carries 2 & 4.
	if (data && data.source === 'zling' && typeof data.driver === 'number') {
		...
		rover.zling.actual_rpm_by_id.last_updated_ts = data.timestamp || Date.now();
	}
};
