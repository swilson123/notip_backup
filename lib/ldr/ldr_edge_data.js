...
function _broadcast_display(white_rabbit) {
    ...
    // The LDR streams hundreds of packets/sec, but the browser can't redraw the
    // full canvas that fast — broadcasting on every packet floods the socket
    // faster than chromium can render, so the backlog (and visible lag) only
    // grows the longer the mission runs. Gate to display_broadcast_interval_ms.
    const now = Date.now();
    if (white_rabbit.ldr.display_last_broadcast_ms && now - white_rabbit.ldr.display_last_broadcast_ms < white_rabbit.ldr.display_broadcast_interval_ms) return;
    white_rabbit.ldr.display_last_broadcast_ms = now;
