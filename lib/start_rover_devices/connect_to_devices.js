var connect_to_devices = function (rover) {
    ...
    if (rover.rplidar.connected === false && !rover.rplidar.reconnecting) {
    ...
    if (rover.realsense.connected === false) {
        rover.connect_to_realsense(rover);
    }

    if (rover.imu_data.connected === false) {
        rover.connect_to_imu(rover);
    }

    if (!rover.lcd_screens.write_to_lcd_interval) {
        rover.connect_to_screens(rover);
    }

    if (rover.irlock.connected === false && !rover.irlock.connecting) {
        rover.connect_to_irlock(rover);
    }
};
