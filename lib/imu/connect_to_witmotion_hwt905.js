...
// --- Config-write protocol (separate from the 0x55-headed data stream above) ---
// Host→sensor command frame, per WitMotion's own SDK (WITMOTION/WitStandardProtocol_JY901,
// Linux_C/normal/REG.h): [0xFF][0xAA][register][value_low][value_high]. Registers relevant
// here — SAVE persists to flash, KEY unlocks writes, AXIS6 selects the Z-axis algorithm:
//   AXIS6 = 0 → 9-axis: Yaw is fused from the magnetometer, absolute, stable across power
//                       cycles (this is the chip's factory default).
//   AXIS6 = 1 → 6-axis: Yaw is gyro-integration only. Per WitMotion's manual (section 2.4.10,
//                       "6-axis/9-axis Algorithm") this exists to dodge magnetic interference,
//                       but the tradeoff is explicit in section 2.3.4 ("Reset Z-axis Angle"):
//                       in 6-axis mode "when the module is powered on, the Z axis will
//                       automatically return to 0" — i.e. it forgets true heading every boot,
//                       which is why the offset has needed re-declaring each time.
const REG_KEY   = 0x69;
const REG_AXIS6 = 0x24;
const REG_SAVE  = 0x00;
const KEY_UNLOCK        = 0xB588;
const ALGORITHM_9AXIS   = 0x0000;
const ALGORITHM_6AXIS   = 0x0001;
const SAVE_PARAM        = 0x0000;
const WIT_CMD_GAP_MS    = 200; // WitMotion units need a gap between config writes or they drop bytes

function wit_write_reg(serial, reg, value) {
    serial.write(Buffer.from([0xFF, 0xAA, reg, value & 0xFF, (value >> 8) & 0xFF]));
}

// Re-assert the Z-axis (yaw) algorithm on every connect so a boot never silently runs in
// whichever mode the chip last happened to be in. Config-gated via
// imu.witmotion_hwt905.algorithm ("9-axis" default | "6-axis") so it can be rolled back if
// the drivetrain's magnetic field turns out to make 9-axis unusable near the motors.
function assert_algorithm(white_rabbit, cfg) {
    if (cfg.enforce_algorithm_on_connect === false) return;
    const want_6axis = cfg.algorithm === '6-axis';
    const value      = want_6axis ? ALGORITHM_6AXIS : ALGORITHM_9AXIS;
    const label       = want_6axis ? '6-axis (gyro-only, resets each boot)' : '9-axis (magnetometer-fused, absolute)';

    white_rabbit.logs.imu.log(white_rabbit, `WitMotion HWT905: asserting ${label} algorithm`);
    wit_write_reg(cfg.serial, REG_KEY, KEY_UNLOCK);
    setTimeout(() => wit_write_reg(cfg.serial, REG_AXIS6, value), WIT_CMD_GAP_MS);
    setTimeout(() => wit_write_reg(cfg.serial, REG_SAVE, SAVE_PARAM), WIT_CMD_GAP_MS * 2);
}
...
var connect_to_witmotion_hwt905 = function (white_rabbit) {
    ...
    if (cfg.port_path) {
        ...
        cfg.serial.on('open', function () {
            ...
            assert_algorithm(white_rabbit, cfg);
