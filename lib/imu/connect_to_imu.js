...
var connect_to_imu = async function (white_rabbit) {
    ...
    try {
        ...
        // Max heading change per 50 ms tick (degrees). When mag=0 the BNO055 EKF can absorb
        // motor EMI as a real heading shift; the tighter cap blocks those spikes.
        // When mag>0 the cap is loose enough to never interfere with deliberate yaw.
        const HDG_TICK_CAP_MAG_OK  = 10.0;  // ~200 °/s — uncapped in practice
        const HDG_TICK_CAP_MAG_BAD =  5.0;  // ~100 °/s — blocks EMI spikes when uncalibrated
        ...
        white_rabbit.imu_data.poll_interval = setInterval(() => {
            try {
                ...
                const mag_cal = (calib >> 0) & 0x03;
                const raw_new = (readInt16LE(eulBuf, 0) / 16.0 + 360) % 360;

                let guarded_raw;
                if (prev_raw_heading !== null) {
                    const diff     = ((raw_new - prev_raw_heading + 540) % 360) - 180;
                    const tick_cap = mag_cal === 0 ? HDG_TICK_CAP_MAG_BAD : HDG_TICK_CAP_MAG_OK;
                    if (Math.abs(diff) > tick_cap) {
                        const capped = (diff > 0 ? 1 : -1) * tick_cap;
                        guarded_raw  = ((prev_raw_heading + capped) + 360) % 360;
                        const now = Date.now();
                        if (now - last_guard_log_ts >= 1000) {
                            last_guard_log_ts = now;
                            console.log(`IMU jump guard: raw ${prev_raw_heading.toFixed(1)}→${raw_new.toFixed(1)}° (${diff > 0 ? '+' : ''}${diff.toFixed(1)}°) capped to ${capped > 0 ? '+' : ''}${capped.toFixed(1)}° — mag=${mag_cal}`);
                            white_rabbit.logs.imu.log(white_rabbit, `jump guard: raw ${prev_raw_heading.toFixed(1)}→${raw_new.toFixed(1)}° diff=${diff.toFixed(1)}° capped=${capped.toFixed(1)}° mag=${mag_cal}`);
                        }
                    } else {
                        guarded_raw = raw_new;
                    }
