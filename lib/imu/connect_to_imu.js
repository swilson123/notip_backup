...
// a fresh calibration (e.g. after relocating the white_rabbit to a different magnetic environment).
...
var connect_to_imu = async function (white_rabbit) {
    ...
    try {
        ...
        if (saved_cal) {
            ...
            white_rabbit.logs.imu.log(white_rabbit, 'BNO055: restored saved calibration from disk');
        } else {
            console.log('BNO055: no saved calibration on disk — will save when sensor reaches full cal (sys=3)');
            white_rabbit.logs.imu.log(white_rabbit, 'BNO055: no saved calibration on disk — will save when sensor reaches full cal (sys=3)');
        }
        ...
        // If this is a reconnect (disconnect_ts was set), announce the star's return.
        if (white_rabbit.imu_data.disconnect_ts) {
            var dark_s = ((Date.now() - white_rabbit.imu_data.disconnect_ts) / 1000).toFixed(0);
            white_rabbit.imu_data.disconnect_ts   = null;
            white_rabbit.imu_data._reconnect_count = 0;
            console.log('BNO055: reconnected after ' + dark_s + 's');
            white_rabbit.logs.imu.log(white_rabbit, 'BNO055: reconnected after ' + dark_s + 's');
            if (white_rabbit.voice) white_rabbit.voice.say('IMU restored.');
        }
        ...
        white_rabbit.imu_data.poll_interval = setInterval(() => {
            try {
                ...
                white_rabbit.imu_data.heading_raw = (readInt16LE(eulBuf, 0) / 16.0 + 360) % 360;
                white_rabbit.imu_data.heading = (white_rabbit.imu_data.heading_raw + white_rabbit.imu.compass_offset_deg + 360) % 360;
                white_rabbit.imu_data.roll    = readInt16LE(eulBuf, 2) / 16.0;
                white_rabbit.imu_data.pitch   = readInt16LE(eulBuf, 4) / 16.0;

                white_rabbit.imu_data.quaternion = {
                ...
                white_rabbit.imu_data.gravity = {
                    x: readInt16LE(grvBuf, 0) / 100.0,
                    y: readInt16LE(grvBuf, 2) / 100.0,
                    z: readInt16LE(grvBuf, 4) / 100.0,
                };

                white_rabbit.imu_data.temperature_c = tmpBuf[0];

                white_rabbit.imu_data.calibration = {
                    system: (calib >> 6) & 0x03,
                    gyro:   (calib >> 4) & 0x03,
                    accel:  (calib >> 2) & 0x03,
                    mag:    (calib >> 0) & 0x03,
                ...
                if (!calibration_saved_this_session && white_rabbit.imu_data.calibration.system === 3) {
                    try {
                        ...
                        if (save_calibration_to_disk(calDataBuf)) {
                            ...
                            white_rabbit.logs.imu.log(white_rabbit, 'BNO055: saved calibration to ' + CAL_FILE_PATH);
                        }
                    } catch (err) {
                ...
                if (white_rabbit.imu_data.timestamp - last_log_ts >= 1000) {
                    ...
                    const c = white_rabbit.imu_data.calibration;
                    white_rabbit.logs.imu.log(white_rabbit,
                        `heading:${white_rabbit.imu_data.heading.toFixed(1)} pitch:${white_rabbit.imu_data.pitch.toFixed(1)} roll:${white_rabbit.imu_data.roll.toFixed(1)} ` +
                        `temp:${white_rabbit.imu_data.temperature_c}c cal(sys:${c.system} gyro:${c.gyro} accel:${c.accel} mag:${c.mag})`
                    );
                    console.log(`IMU: heading=${white_rabbit.imu_data.heading.toFixed(1)} pitch=${white_rabbit.imu_data.pitch.toFixed(1)} roll=${white_rabbit.imu_data.roll.toFixed(1)} ` +
                        `temp=${white_rabbit.imu_data.temperature_c}C calib(sys:${c.system} gyro:${c.gyro} accel:${c.accel} mag:${c.mag})`
                    );
                      //console.log("Heading: "+white_rabbit.get_heading(white_rabbit));
                }
                ...
                white_rabbit.imu_data.poll_interval = null;

                // Stamp when the star went dark. The last known heading, pitch, and
                // roll remain on white_rabbit.imu_data — the knowledge Noah had is
                // still there. get_heading/get_pitch/get_roll will fall back to
                // Pixhawk ATTITUDE automatically while the primary is dark.
                if (!white_rabbit.imu_data.disconnect_ts) {
                    white_rabbit.imu_data.disconnect_ts = Date.now();
                    white_rabbit.logs.imu.log(white_rabbit, 'BNO055: I2C read error — navigating by last known star: ' + err.message);
                    if (white_rabbit.voice) white_rabbit.voice.say('Navigating by the last known star. Reconnecting.');
                }

                // Exponential backoff reconnect: 5 s → 10 s → 20 s → 30 s max.
                var count = white_rabbit.imu_data._reconnect_count || 0;
                var delay_ms = Math.min(5000 * Math.pow(2, count), 30000);
                white_rabbit.imu_data._reconnect_count = count + 1;
                console.log('BNO055: scheduling reconnect #' + (count + 1) + ' in ' + (delay_ms / 1000).toFixed(0) + 's');
                setTimeout(function () { connect_to_imu(white_rabbit); }, delay_ms);
            }
        }, 50);

        // att_pos_mocap (external-nav) feed to the Pixhawk is disabled: its
        // hardcoded position (0,0,0) was pinning the EKF to the origin and
        // crowding out the real GPS. Heading now comes straight from the BNO055
        // via get_heading()'s direct-IMU fallback.

    } catch (err) {
        console.log('BNO055 connect error:', err.message);
        white_rabbit.imu_data.connected = false;

        // Initial connection failure — chip not present or I2C bus not ready.
        // Retry with backoff so a momentary boot-time glitch doesn't leave Noah
        // permanently without his inner ear.
        if (!white_rabbit.imu_data.disconnect_ts) {
            white_rabbit.imu_data.disconnect_ts = Date.now();
        }
        var count = white_rabbit.imu_data._reconnect_count || 0;
        var delay_ms = Math.min(5000 * Math.pow(2, count), 30000);
        white_rabbit.imu_data._reconnect_count = count + 1;
        console.log('BNO055: connect failed — retry #' + (count + 1) + ' in ' + (delay_ms / 1000).toFixed(0) + 's');
        setTimeout(function () { connect_to_imu(white_rabbit); }, delay_ms);
    }
};
