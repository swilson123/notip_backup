    ...
    try {
        ...
        white_rabbit.imu_data.poll_interval = setInterval(() => {
            try {
                ...
                const yaw_raw = readInt16LE(angBuf, 4); // pre-scale register word, for diagnostics
                ...
                // No per-tick rate limiter: the HWT906 outputs a clean absolute heading and a hand-spin
                // easily exceeds any sane cap, which would compress the reading. Trust the sensor directly.
                ...
                if (white_rabbit.imu_data.timestamp - last_log_ts >= 1000) {
                    ...
                    // yaw_raw/yaw_deg exposed for scaling diagnosis: spin the rover slowly through a known
                    // angle and confirm yaw_deg tracks 1:1. If a 180° physical turn moves yaw_deg far less,
                    // the magnetometer needs calibration (hard/soft-iron distortion), not a code change.
                   ...
                   // console.log(`WitMotion IMU: heading=${white_rabbit.imu_data.heading.toFixed(1)} raw=${white_rabbit.imu_data.heading_raw.toFixed(1)} yaw_deg=${yaw_deg.toFixed(1)} yaw_reg=${yaw_raw} pitch=${white_rabbit.imu_data.pitch.toFixed(1)} roll=${white_rabbit.imu_data.roll.toFixed(1)} temp=${white_rabbit.imu_data.temperature_c.toFixed(1)}C`);
                }
