...
function apply_segment(white_rabbit, start_lat, start_lng, start_raw, end_lat, end_lng, end_raw, odo_dist_m, source) {
    if (!white_rabbit.imu || white_rabbit.imu.auto_calibrate === false) return false;
...
// is captured with a fresh offset.
compass_calibration.calibrate_undock_segment = function (white_rabbit, start, odo_dist_m) {
    ...
    return apply_segment(white_rabbit, start.lat, start.lng, start.raw_heading,
        lat, lng, raw, odo_dist_m || 0, 'undock');
};
