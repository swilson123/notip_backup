...
var compass_calibration = function (white_rabbit) {
    ...
    // Source 2 — Pixhawk EKF heading (VFR_HUD). DISABLED by default: the EKF
    // heading is already absolute and ArduPilot already fuses GPS course into its
    // yaw, smoothed and quality-gated. Layering another GPS-derived offset on top —
    // ungated and unsmoothed — injects GPS noise straight into a heading that was
    // steady, which is worse when GPS is marginal. Trust the EKF heading raw
    // (pixhawk_offset_deg = 0) unless imu.calibrate_pixhawk_heading is explicitly
    // turned on. Calibrate the Pixhawk's compass in Mission Planner instead.
    if (white_rabbit.imu && white_rabbit.imu.calibrate_pixhawk_heading === true) {
        calibrate_source(white_rabbit, {
            id:         'pixhawk',
