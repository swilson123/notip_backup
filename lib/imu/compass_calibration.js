...
var compass_calibration = function (white_rabbit) {
    ...
    // Sidewalk-follow gate open — vision steering corrections mean commanded
    // steering no longer implies Noah is actually driving straight, so the
    // GPS-bearing-vs-compass sample can't be trusted. Drop any in-progress
    // segment and wait until the gate closes again.
    if (white_rabbit.mission && white_rabbit.mission.sidewalk_follow_active === true) {
        _snapshot = null;
        return;
    }
