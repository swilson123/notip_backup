		...
		if (front_blocked) {
			...
			if (!avoidance_turn.sidewalk_wait_since) {
				...
				if (white_rabbit.mission.package_delivered) {
					// On the return leg — package is already delivered. Don't say "Delivering here."
					announce('Path blocked. Resuming return.', 'sw_return_blocked', true);
					white_rabbit.logs.avoid_object.log(white_rabbit, 'Sidewalk obstacle on return did not clear in ' + block_timeout + 'ms — releasing path_clear to continue breadcrumb return.');
				} else {
