var avoid_object = function (white_rabbit) {
	...
	function find_zone(zone_number) {
		for (let i = 0; i < white_rabbit.zones.length; i++) {
			if (white_rabbit.zones[i].zone === zone_number) {
				var z = white_rabbit.zones[i];
				// Stale LiDAR data is not knowing — treat the zone as clear rather than
				// blocking Noah on the ghost of a past obstacle.
				if (z.timestamp && (Date.now() - z.timestamp) > LIDAR_STALE_MS) {
					return Object.assign({}, z, { light: 'green' });
				}
				return z;
			}
		}
	...
	function reset_avoidance_state() {
		...
		avoidance_turn.last_voice_key = null;
		avoidance_turn.sidewalk_wait_since = null;
	}

	// Speak a short avoidance step at most once per distinct key, so the same
	// line isn't repeated on every lidar frame while the state holds. The key
	// changes as the plan changes (new turn direction/angle, creep, resume),
	// which lets the next genuine step announce.
	function announce(line, key, urgent) {
		let st = white_rabbit.mission.avoidance_turn;
		if (!st || st.last_voice_key === key) return;
		st.last_voice_key = key;
		if (white_rabbit.voice) white_rabbit.voice.say(line, !!urgent);
	}

	function meters_phrase(m) {
		let r = Math.round(m);
		return r + (r === 1 ? ' meter' : ' meters');
	}

	// Estimated rotation for the chosen escape zone = the angle to that zone's
	// center, used only for the spoken "spinning X degrees" heads-up.
	function planned_turn_degrees(zone_number) {
		let z = find_zone(zone_number);
		return z ? Math.round(Math.abs(get_zone_center(z))) : 0;
	}
	...
	function resume_mission(reason) {
		...
		if (white_rabbit.voice) white_rabbit.voice.say('Path clear. Resuming mission.');
	...
	function get_yaw_to_waypoint() {
		...
		// Use the stable believed heading for the direction decision — a single-tick
		// compass flicker should not flip left/right avoidance direction mid-maneuver.
		let white_rabbit_heading = white_rabbit.heading_belief
			? white_rabbit.heading_belief(white_rabbit)
			: white_rabbit.get_heading(white_rabbit);
	...
	function find_nearest_green_zone(yaw_to_waypoint) {
		...
		// --- God variable reads for richer zone scoring ---

		// 1. GPS quality: if fix is degraded, yaw_to_waypoint is unreliable —
		//    fall back to pure proximity + clearance, ignore waypoint direction.
		let gps_raw   = white_rabbit.robot_data && white_rabbit.robot_data.GPS_RAW_INT;
		let fix_type  = gps_raw && typeof gps_raw.fix_type  === 'number' ? gps_raw.fix_type  : 3;
		let eph_cm    = gps_raw && typeof gps_raw.eph       === 'number' ? gps_raw.eph       : 0;
		let gps_good  = fix_type >= 3 && eph_cm < 300;   // 3D fix, < 3m horizontal precision
		let waypoint_weight = gps_good ? 0.25 : 0.0;     // zero out waypoint pull when GPS is bad

		// 2. Camera obstacle direction: if RealSense sees an in-path object,
		//    its clock position gives a directional hint — prefer turning away from it.
		//    Positive obstacle_center → obstacle is right of center → prefer turning left.
		let camera_obstacle_center_deg = 0;
		if (white_rabbit.realsense && Array.isArray(white_rabbit.realsense.objects)) {
			let closest_in_path = null;
			white_rabbit.realsense.objects.forEach(function (obj) {
				if (obj.in_rover_path && typeof obj.distance_m === 'number') {
					if (!closest_in_path || obj.distance_m < closest_in_path.distance_m) {
						closest_in_path = obj;
					}
				}
			});
			if (closest_in_path && typeof closest_in_path.x_angle_deg === 'number') {
				camera_obstacle_center_deg = closest_in_path.x_angle_deg;
			}
		}
		// Penalty added to zones on the SAME side as the obstacle — steer away from it.
		// Scaled 0-20 degrees of equivalent angular cost.
		let camera_bias_weight = 20;

		// 3. Max green-zone clearance: for the depth penalty calculation.
		let max_green_dist_mm = 500;   // floor so we never divide by zero
		for (let i = 0; i < white_rabbit.zones.length; i++) {
			let z = white_rabbit.zones[i];
			if (z.light === 'green' && z.zone !== 11 && z.zone !== 12
					&& typeof z.distance_mm === 'number' && z.distance_mm > max_green_dist_mm) {
				max_green_dist_mm = z.distance_mm;
			}
		}

		for (let i = 0; i < white_rabbit.zones.length; i++) {
			let zone = white_rabbit.zones[i];
			if (zone.light !== "green") continue;
			if (zone.zone === 11 || zone.zone === 12) continue;

			let zone_center = get_zone_center(zone);

			// How far do I have to rotate? (smaller = better)
			let proximity_score = Math.abs(zone_center);

			// How far is this zone from the waypoint direction?
			// Weighted by GPS quality — zero when GPS is unreliable.
			let waypoint_score = waypoint_weight * Math.abs(normalize_relative_angle(yaw_to_waypoint - zone_center));

			// How open is this zone physically?
			// A zone barely clear (low distance_mm) gets a clearance penalty up to 40°.
			let clearance_penalty = 0;
			if (typeof zone.distance_mm === 'number' && zone.distance_mm > 0) {
				clearance_penalty = Math.max(0, 1.0 - zone.distance_mm / max_green_dist_mm) * 40;
			}

			// Does the camera see the obstacle on this side?
			// If camera_obstacle_center_deg > 0 (obstacle right), penalize right zones.
			// If camera_obstacle_center_deg < 0 (obstacle left), penalize left zones.
			let camera_penalty = 0;
			if (Math.abs(camera_obstacle_center_deg) > 3) {
				let same_side = (camera_obstacle_center_deg > 0 && zone_center > 0)
				             || (camera_obstacle_center_deg < 0 && zone_center < 0);
				if (same_side) {
					camera_penalty = camera_bias_weight * Math.min(1.0, Math.abs(camera_obstacle_center_deg) / 30);
				}
			}

			let total_score = proximity_score + waypoint_score + clearance_penalty + camera_penalty;
	...
	if (white_rabbit.mission && !white_rabbit.mission.package_delivered) {
		...
		// Only measure movement when GPS quality is locked — jitter on a cold GPS
		// can fake 1–2 m of movement while stationary and prematurely commit avoidance.
		let _gps_raw = white_rabbit.robot_data && white_rabbit.robot_data.GPS_RAW_INT;
		let _gps_quality_ok = _gps_raw && _gps_raw.fix_type >= 3 && _gps_raw.eph < 500;
	...
	// ----- Sidewalk mode: stop-and-wait, never steer off the path -----
	// On the sidewalk (between the >90° gate waypoints, outbound, before delivery)
	// we must NOT maneuver into a yard to dodge an obstacle. Stop, tell people what
	// we're doing, and wait. If it doesn't clear within the block timeout, deliver
	// here and head home on the breadcrumb trail (run_mission's fallback delivery).
	if (white_rabbit.mission.sidewalk_follow_active === true && !white_rabbit.mission.package_delivered) {
		// Deliver-here already triggered — hand motor control to run_mission's
		// turn-around + delivery; don't stop or maneuver here.
		if (white_rabbit.mission.sidewalk_deliver_triggered) {
			return;
		}
		...
		if (front_blocked) {
			...
			// Confirm the block briefly before announcing, so a single flickery
			// frame doesn't trigger an alert (we're already stopped either way).
			if ((now - avoidance_turn.blocked_since) < FRONT_BLOCKED_CONFIRM_MS) {
				return;
			}

			let block_timeout = (white_rabbit.nav_tuning && white_rabbit.nav_tuning.rs_block_timeout_ms) || 10000;
			if (!avoidance_turn.sidewalk_wait_since) {
				avoidance_turn.sidewalk_wait_since = now;
				white_rabbit.mission.last_object_detected_voice = now;
				if (white_rabbit.intelligence) white_rabbit.intelligence.consider('path_blocked');
				white_rabbit.logs.avoid_object.log(white_rabbit, 'Object on sidewalk — stopping and waiting up to ' + block_timeout + 'ms.');
				announce('Object detected on sidewalk. Waiting.', 'sw_wait_0', true);
			}
			else if ((now - avoidance_turn.sidewalk_wait_since) >= block_timeout) {
				announce('Object did not clear. Delivering here.', 'sw_deliver', true);
				white_rabbit.logs.avoid_object.log(white_rabbit, 'Sidewalk obstacle did not clear in ' + block_timeout + 'ms — delivering here and returning home.');
				// Hand off to run_mission's fallback delivery: it turns around, delivers
				// at the current spot, then follows the breadcrumb trail home to dock.
				white_rabbit.mission.sidewalk_deliver_triggered = true;
				white_rabbit.mission.avoidance_timed_out = true;
				reset_avoidance_state();
			}
			else {
				// Reassure with a periodic "Waiting." (new key every ~5s so it repeats).
				announce('Waiting.', 'sw_wait_' + Math.floor((now - avoidance_turn.sidewalk_wait_since) / 5000));
			}
			return;
		}
		else {
			// Front clear. If we had stopped to wait, resume; otherwise carry on.
			if (avoidance_turn.sidewalk_wait_since) {
				resume_mission('Sidewalk obstacle cleared — resuming mission.');
			}
			else {
				reset_avoidance_state();
				white_rabbit.mission.path_clear = true;
	...
	if (white_rabbit.mission.path_clear) {
		...
		white_rabbit.mission.last_object_detected_voice = now;
		if (turn_target) {
			let deg = planned_turn_degrees(turn_target.target_zone);
			...
			announce('Object detected. Spinning ' + turn_target.direction + ' ' + deg + ' degrees.', 'turn:' + turn_target.direction + ':' + deg, true);
		}
		else {
			let msg = 'Object detected but no non-front green zone is available yet. Holding position.';
			console.log(msg);
			white_rabbit.logs.avoid_object.log(white_rabbit, msg);
			announce('Object detected. Path blocked, holding.', 'hold', true);
		}
		return;
	}
	...
	if (front_blocked) {
		...
		if (should_reevaluate) {
			...
			// Announce the direction actually committed (it may keep the prior
			// direction to avoid left/right ping-pong), not just the fresh pick.
			if (avoidance_turn.direction && avoidance_turn.target_zone) {
				let deg = planned_turn_degrees(avoidance_turn.target_zone);
				announce('Spinning ' + avoidance_turn.direction + ' ' + deg + ' degrees.', 'turn:' + avoidance_turn.direction + ':' + deg);
			}
			else {
				announce('Path blocked, holding.', 'hold');
			}
		}
	...
	avoidance_turn.aligned_since = null;
	avoidance_turn.settling_since = null;
	announce('Crawling forward ' + meters_phrase(CREEP_CLEARANCE_DISTANCE_M) + '.', 'creep');
	move_forward_slowly();
}
