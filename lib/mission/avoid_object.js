var avoid_object = function (rover) {
	...
	function resume_mission(reason) {
		...
		console.log(reason);
		rover.logs.avoid_object.log(rover, reason);
	...
	function get_current_heading() {
		return rover.get_heading(rover);
	}
	...
	function is_front_blocked() {
		...
		if (rover.realsense && rover.realsense.vision_zones &&
				!(rover.mission && rover.mission.package_delivered)) {
			...
			if (fresh) {
				...
				for (let i = 0; i < front_zones.length; i++) {
					let vz = rover.realsense.vision_zones[front_zones[i]];
					if (vz && vz.light === 'red') {
						return true;
					}
				}
	...
	function choose_turn_target(yaw_to_waypoint) {
		...
		// Positive zone center = right side of rover (clock zones 1-5); negative = left (zones 6-10).
		return {
	...
	// Global avoidance timeout: if the rover has been in avoidance continuously for too long,
	...
	if (avoidance_turn.avoidance_started_at) {
		let avoidance_timeout_ms = (rover.nav_tuning && rover.nav_tuning.avoidance_timeout_ms) || 30000;
		if (now - avoidance_turn.avoidance_started_at >= avoidance_timeout_ms) {
			rover.mission.avoidance_timed_out = true;
			rover.logs.avoid_object.log(rover, 'Avoidance timeout after ' + avoidance_timeout_ms + 'ms — flagging for fallback delivery');
		}
	}
	...
	if (rover.mission.path_clear) {
		...
		if (!avoidance_turn.avoidance_started_at) {
            ...
            if (rover.intelligence) rover.intelligence.consider('avoidance_started');
        }

		if (turn_target) {
			let msg = 'Object detected! Turning ' + turn_target.direction + ' toward green zone ' + turn_target.target_zone + '.';
			console.log(msg);
			rover.logs.avoid_object.log(rover, msg);
		}
		else {
