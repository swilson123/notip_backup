var avoid_object = function (rover) {
	...
	function is_forward_corridor_clear() {
		...
		// Also check RealSense high-threat objects before declaring corridor clear.
		if (rover.realsense && Array.isArray(rover.realsense.objects) &&
			...
			if (fresh) {
				...
				for (let i = 0; i < rover.realsense.objects.length; i++) {
					...
					if (obj.in_rover_path && obj.threat_level === 'high' &&
							obj.distance_m <= stop_dist && obj.confidence >= 0.5) {
						return false;
					}
				}
			}
	...
	function is_front_blocked() {
		...
		// Also check RealSense high-threat objects. Camera (0.406 m) sees low obstacles the lidar (0.61 m) misses.
		if (rover.realsense && Array.isArray(rover.realsense.objects) &&
				!(rover.mission && rover.mission.package_delivered)) {
			let vision = rover.realsense.vision || {};
			let detection = rover.realsense.path_detection;
