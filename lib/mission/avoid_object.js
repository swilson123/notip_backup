var avoid_object = function (rover) {
	...
	function is_forward_corridor_clear() {
		...
		// Also check RealSense vision zones (10→11, 11→12, 12→1, 1→2) before declaring corridor clear.
		// Each zone requires 1 second of sustained detection before its light activates.
		// yellow or red = not clear (yellow means proceed slowly; red means stop).
		if (rover.realsense && rover.realsense.vision_zones &&
			...
			if (fresh) {
				let corridor_zones = [10, 11, 12, 1];
				for (let i = 0; i < corridor_zones.length; i++) {
					let vz = rover.realsense.vision_zones[corridor_zones[i]];
					if (vz && vz.light === 'red') {
						return false;
					}
				}
